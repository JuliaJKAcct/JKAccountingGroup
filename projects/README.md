...
| [`knowledge-hub/`](./knowledge-hub/) | One on-brand, searchable **index page** for the whole firm — every SOP, every client, and the firm's downloadable **templates** in one place, generated straight from `sops/` + `client-intelligence/` so it never drifts. A "review Hub" (shows everything, with status badges) that later publishes only the *ready* documents to the Odoo team site. Built on Atlas with the `impeccable` skill. | Active |
...
| [`lilian-notebook/`](./lilian-notebook/) | **Lilian's personal notebook** — the firm's hard-won lessons (an agency's real behaviour, a platform's traps, what a filing costs), each written as the rule to follow next time, generated from `notes/*.md` into one searchable Atlas page she has bookmarked, plus a card in the Hub. Written via the [`lilian-notebook`](../.claude/skills/lilian-notebook/) skill, which also tells Claude to **propose** entries unprompted. Hers by design — the repo's one deliberate per-person space; firm-wide procedure still belongs in `sops/`. | Active |
