...
| [`marketing/consultation-booking/`](./marketing/consultation-booking/) | The booking front door — routes visitors to the free 10-min phone discovery call or the paid 1-hour $150 consultation, on two Odoo Appointments calendars with different availability. Online, EN/RU. | Active |
...
| [`pre-return-review/`](./pre-return-review/) | The firm's **analysis companion** (Lilian's "tax preparer") — its purpose and standing requirements, plus [`method.md`](./pre-return-review/method.md): **how we analyse anything**, not only tax organizers. Powered by the [`organizer-review` skill](../.claude/skills/organizer-review/). Reviews are delivered in chat, never committed. | Active |
...
| [`knowledge-hub/`](./knowledge-hub/) | One on-brand, searchable **index page** for the whole firm — every SOP, every client, Lilian's Notebook, and the firm's downloadable **templates** in one place, generated straight from `sops/` + `client-intelligence/` + `lilian-notebook/` so it never drifts. A "review Hub" (shows everything, with status badges) that later publishes only the *ready* documents to the Odoo team site. Built on Atlas with the `impeccable` skill. | Active |
...
| [`lilian-notebook/`](./lilian-notebook/) | **Lilian's personal notebook** — the firm's hard knowledge (how a system behaves, what something costs, what's inside a fee, how to carry out a procedure), each written as the rule to follow next time, **deliberately small**, generated from `notes/*.md` into one searchable Atlas page she has bookmarked, plus a card in the Hub. Written via the [`lilian-notebook`](../.claude/skills/lilian-notebook/) skill — which tells Claude 🛑 **never to propose one**: she asks, or it doesn't happen (2026-08-13). Hers by design — the repo's one deliberate per-person space; firm-wide procedure still belongs in `sops/`. | Active |
| [`tax-returns/`](./tax-returns/) | **The working papers behind every return we prepare** — what went on each line, where each figure came from, what was decided and why, and what carries into next year. The **only** place in the repo that holds client dollar figures (Client Intelligence deliberately does not). **Not published to the Hub.** | Active |
| [`hiring-bookkeeper-test/`](./hiring-bookkeeper-test/) | The firm's **practical bookkeeper hiring test** — three months of mock QuickBooks books for a fictional construction company with 21 deliberate errors seeded across the bank reconciliation, the month-end close and the financial statements, plus the bank statements, the candidate brief and the evaluator's answer key. The books are **generated from code and assert their own tie-outs**, so every discrepancy a candidate finds was put there on purpose. All mock data — no client information. | Active |
