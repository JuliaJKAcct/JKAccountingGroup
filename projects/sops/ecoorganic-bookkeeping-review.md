...
- 🔒 **The client's own subcontractors and payees are written by ROLE, not by
  ...
  U-Haul, Upwork, Thumbtack — and Lilian ruled that explicitly allowed on 2026-08-26** when
...
- **The number-prefix chart of accounts is the target, not the first step.**
  ...
  ⚠️ **The account-line questions wait on the SAME TRIGGER — the cleared backlog —
  but they are not part of the renumbering and must not slip with it:** the
  `Software` ↔ `Dues & subscriptions` overlap and everything tangled into it
  (**row 14**), and the individual paid by Zelle out of `Software` (**row 16**).
  They wait for the same reason renumbering does — settling them means reclassifying
  history, which you do not want to be doing while the backlog is still moving.
...
| 14 | 🔵 **`Software` and `Dues & subscriptions` overlap and the line between them is not clear — REVIEW AFTER THE CLEANUP.** Lilian's read, 2026-08-26: *"siento que son muy similares entre sí… no creo que quede muy clara la diferencia entre una y otra"*. 🔑 **She asked Julia the same question at the 2026-07-22 review, and Julia's answer is the starting point — it was never written down.** Julia's line: **`Software` = actual applications you use for work; `Dues & subscriptions` = memberships and other monthly payments that are NOT a subscription to software you work in** — the account came from memberships historically, and nowadays almost everything is billed monthly, which is why it blurs. She said plainly that **"they're very close"**. **The open question is therefore narrower than it looks:** adopt Julia's line as a written rule, or merge the two accounts. ✅ **The history supports her framing.** The only unambiguous *dues* item is a **trade-association membership**; everything else in that account is ordinary SaaS (an AI subscription, a credit-monitoring subscription). One platform (**Thumbtack**) is **split across both accounts**. ⚠️ **Two things widen the question beyond two accounts:** an **advertising/lead-generation** spend sits in `Software` while `Advertising & Marketing` exists — and part of that same platform's charges is the split above, so it may belong in neither; and a few **QuickBooks transaction/verification fees** sit in `Software` while `QuickBooks Payments Fees` exists, **with a QBO subscription ACH sitting in the fees account in return** — crossed both ways. ⓘ **Most of the Intuit block in `Software` is the QuickBooks Online SUBSCRIPTION and is correctly placed** — do not sweep the vendor wholesale. **Do not start this mid-cleanup:** deciding the line means reclassifying history, and the backlog has to be worked first. | Lilian | To study |
| 15 | 🔵 **Renumber the chart of accounts to the firm's number-prefix grammar — AFTER THE CLEANUP.** Lilian asked for this on 2026-08-26, using **the number sequence already recorded in this runbook's "Chart of accounts conventions"** (100s assets · 200s liabilities · 300s equity · 400s income · 500s COGS · 600s opex · 800s other income · 901 depreciation · 997/998/999 triage). **This confirms the sequencing this SOP already states** — *classify first, restructure the chart second* — so it is not a change of plan, it is her putting a date on the second half. ⚠️ **The 2025 safety rules govern it:** renames and renumbers are retroactive and harmless, but **never merge** an account with 2025 activity and **never change its type** — create a new one and reclassify 2026 into it. ⚠️ **UNCONFIRMED READING — her words were "shadow of accounts".** Read as **chart of accounts**, because "la secuencia de números que ya conoces" points at the numbering grammar this runbook already carries. **That is an inference from a dictated phrase, not something she confirmed**, and it commits her to a named piece of work — so it is queued as a question **to put to Lilian directly** rather than left to be noticed here. | Lilian | To study |
| 16 | 🔵 **The 1099 sweep is scoped by ACCOUNT plus one payment method — a payee paid another way out of a non-labor account would be missed.** Found 2026-08-26 while checking row 14: a payee whose name reads like a **person** is coded to `Software`, where under rule 17 a person paid in their own name is `Contract labor`. ⓘ **The instance itself is minor and is NOT the point** — one payment, well under the $2,000 threshold, and **dated 2025, which is closed**, so it is a flag rather than a correction (same as the 2025 owner-draw entries noted in rule 3). ⚠️ **What it exposes is the sweep's shape.** Checklist item 8 walks *Outside services · Contract labor · Auto repair* **and Zelle** — so this one *is* caught, but only by the payment-method half. **A person paid by check or card out of a non-labor account would not be**, because nothing in the sweep looks at *who the payee is*. Worth deciding whether the sweep should start from **payees who look like people** rather than from an account list. ⓘ **And the counts here describe what is POSTED** — row 11's backlog means the 2026 picture is incomplete. | Lilian | To study |
