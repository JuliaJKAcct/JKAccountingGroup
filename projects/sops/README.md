...
├── ecoorganic-bookkeeping-review.md             ← Ecoorganic monthly bookkeeping rules + review checklist + the monthly CT sales-tax (OS-114) filing
...
| [`ecoorganic-bookkeeping-review.md`](./ecoorganic-bookkeeping-review.md) | Per-client runbook: Ecoorganic's monthly bookkeeping categorization rules (checks/deposits, gas threshold, COGS, owner-personal patterns), COA conventions, reviewer checklist, and open-decisions log — **plus the monthly Connecticut sales-tax filing** (OS-114 as a zero return via CT DRS myconneCT: the steps, the Drive filing convention, the due date and the $50 late floor, and the unresolved question of *why* it is zero). Rules only — client figures and logins stay in client systems, referenced by Drive link. |
