...
|---|---|---|---|
| QuickBooks Online (Gossip Miami) | The client's ledger — the firm has **user access only**, granted 2026-07-29, to build the P&L | _(n/a — the firm is an invited user)_ | Access was granted **for the financial-statement work**, not for ongoing bookkeeping. ⚠️ **BOTH company bank accounts are DISCONNECTED**, so the transactions since never loaded. **Only the client can reconnect** — the "**Fix now**" prompt in his own QuickBooks. **Update 2026-08-11: only ONE of the two can ever be reconnected** — the checking account is closed, and the client is **already attempting the reconnection** _(in progress, Lilian)_, **so stop asking for it**. ⚠️ **His attempt is stuck on TD's consent screen with no selectable account** (§6) — a **closed account cannot be enrolled**, and TD only offers accounts on the **primary banking profile**. **Verify in QuickBooks rather than chasing**, the firm has user access (§6) |
...
### Licenses & other filings — municipal permits, renewed during the year
...
- **Applies?** **Yes.** The 2025 cleanup surfaced **three permit-renewal payments** the client
  ...
  one paid through the **EGOV / NIC** online government-payment portal.
- **Where:** the September pair are renewals **in the city of NORTH MIAMI BEACH** _(client's own
  words)_. §1 records the salon as being "in Miami" — that came from us, not from the client, so
  **treat North Miami Beach as the better evidence of where this business actually sits** and check
  the address in Double before relying on either.
...
- **A recurring payment to an individual has to be identified before it's categorized** — the same
  ...
  §4.)_
- **The unexplained small charges on this client's accounts have all turned out to be MUNICIPAL
  PERMIT RENEWALS.** All three transactions the 2025 cleanup could not read on its own came back the
  same way _(client, 2026-08-11)_ — two in September, one through the EGOV/NIC portal in October.
  **Check that explanation first** on any similar charge, and categorize to taxes & licences rather
  than to a general expense account.
- **A deposit into the account is not automatically a sale — for this client it is often the opposite.**
  ...
  automatic charge overdrew the account and they covered it themselves)_. That is an **owner
  contribution — equity, not income** — and a cleanup that books deposits as sales by default will
  overstate revenue and the tax with it. **Ask about every deposit that doesn't match the pattern of
  the salon's takings**, rather than assuming.
...
- ✅ **CLOSED 2026-08-11 — all five transaction questions.** Two permit renewals in North Miami Beach (September), one permit renewal via EGOV/NIC (October), the contractor checks, and the deposit that is an **owner contribution**, not revenue. **Nothing on the transaction list is outstanding** — the categorization can proceed on these.
...
- **WAITING ON THE CLIENT — the credit-card statements covering 2025-12-03 → 2025-12-31**, which sit on the statement closing **2026-01-03**. ⚠️ **This is the one thing he has answered around twice** — he addressed the reconnection both times and the statements neither time, so **the next email asks for the statement and nothing else**. The **reconnection is in progress and is no longer an ask** _(Lilian, 2026-08-11)_. Then the **date business use ended** (§5) — not the sale date, which is a different unknown — before anything on that card is categorized after it.
