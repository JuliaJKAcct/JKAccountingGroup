...
- ✅ **Settled 2026-08-26** *(raised at the 2026-07-22 client review meeting as "Shipping and deliveries")*: truck-rental charges (e.g. U-Haul) for moving tools/materials to a job site → the **`Shipping` (COGS)** account — SOP rule 16. **No account named "Shipping and deliveries" exists**; Lilian named `Shipping` and named it as cost of goods sold. ⚠️ **Same meeting, corrected 2026-08-26:** this file previously recorded that *"software/subscription charges are consolidated into a single QBO account (no per-tool breakout)"*. **That is a misreading of a garbled transcript.** What Lilian actually said is that **the FIRM usually keeps one account for everything that is a subscription** — a remark about our habit across clients, not a statement that this client's charges were consolidated. Ecoorganic has **two** accounts and always did. _(Re-read from the Ping transcript, 2026-07-22 at 112:41–114:00.)_
...
- 🔑 **Categorization rules Lilian set on 2026-08-26**, given as a batch while opening the
  ...
  - **The recurring insulation subcontractor → Outside Labor**, and ✅ **"Outside Labor" means
    `Outside services`** _(settled 2026-08-26 — an earlier direct answer that day said `Contract
    labor`; her fuller instruction the same day is incompatible with that, and no account of either
    name exists but `Outside services` does. See SOP rule 17)_. **This subcontractor invoices as an
    LLC, so it lands in `Outside services`; an individual would land in `Contract labor`.** ⚠️ **Paid by Zelle under an INDIVIDUAL's name while the vendor record is the
  ...
  - **Spray / foam suppliers → Cost of Goods Sold**, because that is the material consumed by the
    ...
    July** — and the answer is **asymmetric**: materials to COGS, subcontractor **labor to the
    labor accounts per SOP rule 17** (an individual → `Contract labor`; an LLC or company →
    `Outside services`) — **opex either way**, a deliberate departure from the firm-wide framework
    that puts both in COGS.
  ...
  - **ATM WITHDRAWALS → the labor accounts, account per SOP rule 17** (an individual →
    `Contract labor`; an LLC or company → `Outside services`), ⚠️ **and from now on the client is asked every single time
  ...
  - **The largest 2026 labor payee is a subcontractor** — so 1099 exposure, and a W-9 is needed.
    ✅ **Settled 2026-08-26: he is an INDIVIDUAL, so `Contract labor`** (SOP rule 17, decisions
    row 9).
...
- ⛔ **The `Artem Personal` bank account is OUT OF SCOPE — do not categorize it, do not clean it**
  ...
  mistake**. Julia's instruction for it, in order: **export its transactions to Excel first** —
...
- 🔴 **QuickBooks' "pair" suggestions are actively producing false transfers on this client, and
  ...
  and the transaction never reaches an income or expense account. Two shapes of damage found in the
  posted 2026 ledger: **ATM check deposits** booked as transfers instead of **revenue**, and **ATM
  cash withdrawals** booked as transfers instead of **labor** — the second also erases the 1099
  exposure the withdrawal carries. The example Lilian brought was a **gas-station fuel charge**
  offered as a transfer against a Zelle payment received a week later, for the same amount, from an
  unrelated person. **Rule: never accept a pair suggestion involving `Artem Personal`** — a fuel
  purchase, an ATM deposit and an ATM withdrawal are not transfers by nature, whatever the amounts
  match. Undo one with **Unpair transaction** on the posted transaction _(2026-08-26)_.
...
- 2026-08-29 — **Weekend sweep (incremental, baseline 2026-08-22→2026-08-29).** Double: the case note 485258 (QuickBooks primary-admin handover) is unchanged — `updatedAt` still 2026-08-06, still waiting on Ishkhan's ID verification. No property changes, no activity-log entries in the window (this covers the 2026-08-26 QuickBooks-cleanup working session recorded above by Lilian directly; the connector itself shows nothing new). Gmail (Ecoorganic/Corganix/Markarian + "CT-941" + "Turo", bounded `after:2026/08/15`–`08/22`): no reply or action found on the CT-941 notice beyond the original CT DRS email; no Turo escalation email found. Ping: no results dated on/after 2026-08-22. **Chase pass:**
  - 🔴 **CT-941 past-due notice (arrived 2026-08-18) — a search of Gmail, bounded ≥2026-08-15, on 2026-08-29, found no reply, filing, or other action on it.** **11 days pending, still not actioned.** No new deadline beyond "past due."
  - 🔴 **QuickBooks primary-admin handover (case note 485258) — unchanged, still waiting on Ishkhan's ID/QR-code step. 23 days pending since last moved (2026-08-06).** No deadline, but the downgrade-forgetting risk (§5) grows every week it sits.
  - **Turo escalation email — a search of Gmail, bounded ≥2026-08-15, on 2026-08-29, did not find it.** Expected by ~2026-08-06; **23 days pending**, still no email, no re-call recorded.
  - **Nine months of 2025 with no sales-tax return on file / two unopened DRS notices — no update found this run.** **17 days pending since discovered (2026-08-12).** Live risk unchanged: an unprotested proposed assessment becomes final — no specific statute date is on record for either notice, because neither has been opened.
  - The zero-filing-basis question stays parked per Lilian's 2026-08-13 instruction (not chased, by design).

- 2026-08-26 — **QuickBooks cleanup opened; nine categorization rules received and the books read against them (Lilian).** Lilian gave the batch of client-specific rules now recorded in §5 and asked what they turn up. Read the live chart of accounts, the 2026 YTD P&L, the AR aging and the posted transaction history through Double (read-only; figures stayed in chat under the two-data-homes rule). **What the rules actually caught, worst first:** ① **false "pair" transfers against `Artem Personal`** — accepted pair suggestions turned ATM **check deposits** into transfers instead of revenue and ATM **cash withdrawals** into transfers instead of labor, so both income and 1099-bearing cost are missing from the P&L (§5 has the mechanism and the rule); ② **spray-foam material sitting in `Supplies & Materials` instead of `Cost of Goods Sold`** — four 2026 purchases from the foam distributors, a material total, which moves **gross profit** and feeds Form 1125-A on the return, though not net income; ③ **the recurring insulation subcontractor and the freelance platform are in each other's accounts** relative to Lilian's rules — the subcontractor sits in *Outside services* and the platform in *Contract labor*, the exact reverse of what she specified. ⚠️ **CORRECTED 2026-08-26 — do not act on this as written:** once *"Outside Labor"* resolved to `Outside services`, **only the platform half stands.** The LLC subcontractor sitting in `Outside services` is **correctly filed — do not move it.** See SOP rule 17; ④ **`Meals` is nowhere near the ~$0 the SOP's rule 3 requires** — it carries a substantial 2026 balance of restaurant/convenience charges that the resolved meals policy sends to owner distributions; ⑤ a **hotel charge posted to the `Travel` PARENT** rather than `Travel:Hotel`, breaking the SOP's no-postings-to-parents rule; ⑥ **`Ask my accountant` is not at $0**, so the close gate fails as written. **Two rules had nothing to correct, and the reason matters more than the finding:** **Amazon** has *zero* posted 2026 transactions — the only Amazon trace in 2026 is an overdraft fee on an `AMAZON CORP / SYF PAYMNT` ACH, which is a payment to an **Amazon store card (Synchrony)** whose purchases never enter the Chase feed at all; and **U-Haul**'s 2026 charges are likewise visible only through their overdraft fees. Both point the same way: **a large 2026 backlog is still unposted in the bank-feed queue** (the client's QuickBooks banking screen showed several hundred pending items), so the ledger the firm can read is not the whole year. **Nothing was written to QuickBooks** — neither the Double MCP nor the QuickBooks MCP can recategorize a transaction, so the output is a worklist Lilian executes in QuickBooks by hand. _(Worked by Lilian.)_

- 2026-08-26 — **Lilian settled the three open categorization decisions, and one answer corrected a rule this file had carried for a day (Lilian).** **Row 12:** the runbook **may name retail chains and platforms** — the client's own payees stay by role. **Row 13:** truck/trailer rental goes to **`Shipping`, the COGS account**, reversing a session's inference that *"a delivery cost is not COGS"*; for this trade, delivering the job's materials to site is part of the cost of the job. **Row 9:** the largest labor payee is an **individual subcontractor → `Contract labor`**. 🔑 **Row 9 is the one worth learning from: it went unanswered the first time because the question named him BY ROLE**, which is the repo convention and is exactly what made it unanswerable in conversation — re-asked **by name** (from the QBO vendor list, which is where the mapping lives) it was answered immediately. **Ask by name in chat; write by role in the repo.** Settling it also produced rule 17's real dividing line, which is **who you are paying, not what they do**: an **individual** → `Contract labor`, an **LLC/company/platform** → `Outside services`. That corrected a one-day-old version of rule 17 which had put the LLC subcontractor in `Contract labor`, and it means **"Outside Labor" is her name for `Outside services`**, not for `Contract labor` as a session had recorded. _(Worked by Lilian.)_
...
- 2026-09-02 — 🔓 **THE 2023 RETURN WAS READ TOO, on Lilian's explicit written authorisation, and it
  fixed a figure the 2025 return needs.** Her scope came with it: *read it for what changes 2025 and
  nothing else — 2023 is closed and we are not modifying it.* **What changes 2025:** the 2023 return
  ends its accumulated-adjustments account at a specific negative figure, **and the 2024 return opened
  that account at ZERO instead** — so the opening balance the 2025 return inherits is **understated**,
  and there are now two defensible numbers for line 1 of that schedule. ⚠️ **Practically it moves one
  line on one schedule and no tax** — but it should be a choice someone makes. **Figures in the
  [working paper](../../tax-returns/ecoorganic-usa-llc/2025-form-1120s.md) §6 E-8.**
  ✅ **It also settled the truck's name: the client was right and the 2024 return is the wrong one.** The
  2023 return names it a **RAM 1500** and the 2024 return a **RAM 2500 with the same VIN** — and the
  client had **already told Julia in September 2024** that the draft said 2500 and should say 1500. The
  2023 signed return carries the correction; the 2024 return, filed seven months later, reverted.
  ⛔ **It does not affect 2025** — the asset was disposed of in 2024 — **but the wrong name is still in
  the 2025 books.** ✅ **Also confirmed: one shareholder in 2023 as well, and zero officer compensation
  in 2023, 2024 and 2025 — a three-year pattern.**
  📋 **And the `Outside Services` question was PARKED, deliberately, on Lilian's instruction:** the 2025
  books stay exactly as Julia categorised them, and the working paper (§D-5) now holds the briefing for
  the summary Lilian will give her. 🔑 **The reason it needs a conversation rather than a decision:
  Julia's own review workbook and Julia's own words in the July review meeting point in opposite
  directions** — the workbook says the outside-services block is install labour that belongs in cost of
...
- 2026-09-02 (evening) — 📗 **The 2025 GENERAL LEDGER arrived, the reconciliation is done, and the
  ...
  _(Worked by Lilian.)_
- 2026-09-02 (evening, later) — 🎯 **CASH BASIS confirmed, the reclassification entry re-cut on EVIDENCE
  rather than suspicion, and four bookkeeping questions written up for Julia instead of being fixed
  (Lilian).** ✅ **The 2025 return is prepared on the CASH basis, because the 2024 return was filed that way**
  — so the client's own cash-basis P&L and balance sheet are the return's source, and the accrual figures
  this firm had been working from move only on income *(the difference is the year-end receivable)*.
  ⚠️ **Receipts are over the small-company threshold, so Schedules L and M-1 are both required.**
  🔑 **She then reset the standard for the year-end entry: only what is EVIDENT goes in it.** The analysis was
  re-graded item by item, and the framing changed for the better — **the test is no longer "is this a Turo
  expense?" but "does the company own a vehicle these parts could go on?"** Against a fleet of a box truck, a
  van and a forklift, every European-marque parts and service vendor fails that test on the fixed-asset
  register alone, with no inference about trips or platforms. **The entry shrank: fuel at out-of-state
  brands, Miami-area parking and two premiums from an unverified insurer all came OUT**, because each needs a
  chain of inferences. **What would settle them is one document — the insurance declaration pages, which name
  the insured vehicles and would settle the other auto policy at the same time.**
  🧭 **Her ceiling test — that the platform's own report caps Turo-related costs — was answered honestly:** it
  caps the HOST side *(the platform's commission)* and does not reach either the company's own rentals as a
  guest or the costs paid to dealers off the platform. **What it does give is the size of the activity.**
  📝 **And four things stay exactly as Julia posted them, by Lilian's decision, written up with names, dates
  and amounts so Julia can settle them in one pass:** payments to named individuals sitting in the owner-draw
  account *(two of whom have their own contract-labour sub-accounts in this very chart of accounts, and one
  is already paid as a subcontractor elsewhere in the year)*; money going out netted inside the contribution
  account; card statement credits and third-party receipts booked as contributions; and the health-insurance
  premiums. 🩺 **On the health insurance:** the insurer is a health-marketplace carrier, the premiums are
  monthly and **stop after April**, and the **above-the-line deduction is blocked by the missing W-2** — a
...
- ✅ **What the search DID turn up, and it corroborates the direction if not the date:**
  - **2026-07-01, to a customer's accounts team, copying Julia:** *"**Ishkhan Markarian initially filed
    the business registration and his son Artem Markarian is the current principal.**"* — independent,
  ...
  - ⚠️ **BOI reports were filed for this company in December 2024 naming the outgoing owner.** **If
    ownership changed on 2025-01-01, whether a beneficial-ownership update was owed — and under what
    rules, given how the reporting regime moved during 2025 — is a question for Lilian, not an assertion
    here.**
...
- 📝 **FOUR ITEMS ARE PARKED FOR JULIA, DELIBERATELY — the return goes forward on the books as they stand**
  *(Lilian, 2026-09-02 evening; full detail with names, dates and amounts in the working paper §D-9)*:
  payments to named individuals in the owner-draw account that look like subcontractor labour; money going
  out netted inside the contribution account; card statement credits and a third-party receipt booked as
...
- 🔴 **Ask the client whether any cash / ATM check deposits are payments of the OPEN INVOICES.**
  The AR aging carries open invoices to three customers, most of it long overdue. Any deposit that
  is really an invoice payment must be applied against the invoice, not booked to Sales — otherwise
  the revenue is counted twice and the receivable never clears.
- **Export `Artem Personal` to Excel, then disconnect it** (Julia's instruction, §5) — the business
  expenses mixed into it get re-entered in QuickBooks manually afterwards, as separate work. Until
  the export exists, the account stays connected and untouched.
- **The 2026 bank-feed backlog is the real gate on this cleanup.** Several hundred items were
  pending in QuickBooks' banking screen and are invisible to Double, so any conclusion drawn from
  the posted ledger — including "there are no Amazon purchases in 2026" — is a statement about what
  is *posted*, not about the year. Work the queue before treating the P&L as complete.
- **Confirm whether an Amazon store card (Synchrony) exists and is outside QuickBooks.** The 2026
  Amazon trace is an ACH payment to `AMAZON CORP / SYF PAYMNT`, which is a card payment, not a
  purchase — so the purchases are on a card the books may never see. Bears on Lilian's
  Amazon → Supplies & Materials rule, which currently has nothing to apply to.
- 🔴 **A CT DRS myconneCT notice arrived 2026-08-18: "Your CT-941 Reconciliation of Withholding CT Tax return is past due."** Not yet reflected in §4's payroll obligations — (Gmail, 2026-08-18). **Still not opened/actioned as of 2026-08-29 — 11 days pending.** Flag to Lilian.
