...
| American Express | Company card — a source the firm asks for every year | n/a | 🔴 **The company HAS an Amex and always has.** The client calls it *"Zakom Amex"* in his own words and files it separately from *"Zakom Checking acc"*; the firm holds `Amex Zakom YearEndSummary_2023.pdf`, `Amex 2024 Zakom.pdf` and `Dec Amex balance stmnt.pdf`. **The firm's own annual request letter asks for the December bank AND credit-card statement to confirm balances** _(Julia to the client, Feb 2023)_. ⛔ **Nothing for 2025 has been provided, and the exposure is far larger than a missing document: the 2025 bank statements show 39 Amex payments across the year** *(working paper §3E ②)*. ⛔ **CORRECTION 2026-09-13: the differing references are PER-PAYMENT confirmation numbers, not card identifiers** — all 39 differ — **so the bank says nothing about how many Amex accounts exist.** 🔗 **A PARALLEL SESSION opened the Amex year-end summaries and the 2024 bank statements the same day and found MORE credit accounts than this row names** — branch `claude/cc-statements-2025-oleg-gddicu`, commit `0447ade`, ⚠️ **UNMERGED at the time of writing.** ⛔ **Its rows carry the evidence, the method and the caveats; they are deliberately NOT paraphrased here** *(a summary firmer than its source is how a withdrawn reading gets re-established)*. **Read it there, and merge it before relying on it.** |
