...
- ✅ **THE FORM 6166 CERTIFICATES ARRIVED — 2026-08-19.** Two documents, **"Form 6166_2024"** and **"Form 6166 Tax 2023"** (IRS Certification of U.S. Tax Residency), were uploaded 2026-08-19 into a Drive folder titled "Forms 8802" that already holds this client's Form 8802 application and its May 2026 payment confirmation. This reads as the IRS-issued certificates for **both** requested years (2023 and 2024) finally arriving — roughly 15 weeks after the 2026-05-06 filing. **This closes the "watch for IRS mail" outstanding item.** _(Found via Google Drive, 2026-08-22 sweep — the file was uploaded to a folder shared with [`aura-remodeling.md`](./aura-remodeling.md)'s AP mailbox, so this fact is recorded here, on the individual applicant's file, not there.)_ ⚠️ **Not yet confirmed:** whether these are the final, complete certificates or a partial response — worth Lilian/Julia opening them to check content, dates and any conditions before treating the matter as fully closed. ⓘ **Checked further 2026-08-29, from metadata only (content not opened — outside this sweep's document-reading permission):** both files are substantial PDFs (~1.05–1.1 MB each), consistent with real multi-page certificates rather than empty placeholders. The **2023** file shows a `viewedByMeTime` of 2026-08-19 (a few hours after upload) — someone on the practice's Drive account has opened it at least once; the **2024** file carries no such timestamp, meaning it may not have been opened at all. This is a partial, metadata-only signal toward "complete" — it does not substitute for a human reading the content for dates and conditions.
 ✅ **CONTENT CONFIRMED 2026-09-12 — both certificates are genuine and complete.** A Drive search this run returned a content snippet inadvertently (an operator error — the search should have used `excludeContentSnippets: true` and did not); this closes the "is it complete or partial" question the 2026-08-29 entry left open: **both PDFs are the IRS's own "Certification of U.S. Tax Residency" letters, dated August 17, 2026** — one for tax year 2023, one for tax year 2024, each with the enclosed Form 6166 residency certificate — not partial or placeholder documents. 🔒 **The identity block the snippet also exposed (both spouses' TINs and the mailing address on the application) was NOT written anywhere and never will be** — this is exactly the leak the [`client-intelligence` skill](../../../.claude/skills/client-intelligence/SKILL.md) warns a content-snippet search can cause; **flagged to Julia/Lilian in this run's report, with the standing reminder to delete this session.** The Form 8802 application itself was also incidentally visible in the same batch and separately confirms the certification was requested for **both 2023 and 2024** in Ihor Naum's name, matching what this file already recorded independently.
...
- 2026-08-29 — **Weekend sweep (incremental, baseline 2026-08-22→2026-08-29).** Double: `list_notes` still 0 notes (matches prior sweeps); zero activity-log entries since baseline. Gmail (`after:2026/08/22`, both names + "Form 6166" + 2848 + duplicate) found nothing beyond the internal weekly-sweep digest — no update on the possible-duplicate-transmission question or a Form 2848 confirmation. Google Drive: re-checked the Form 6166 files' metadata (see §4) — both are substantial PDFs; the 2023 one has been opened once (`viewedByMeTime` 2026-08-19), the 2024 one has not — content itself not opened (outside this sweep's permission). Ping org-wide semantic search returned only unrelated pre-2026-08-22 noise; `resolve_person` still no match. Chase pass: all open items below checked, ages updated; no SOP exists for this client, so no SOP-proposal candidates queued.
- 2026-09-12 — **Weekend sweep (incremental, baseline 2026-08-29→2026-09-12 — the 2026-09-05 run never completed, see `sweep-state.md`).** Double: `list_client_properties` current; `list_notes` still 0; `list_activity_log` (from 2026-08-29) returned zero entries. Gmail bounded `after:2026/08/29` for both names/"Form 6166"/2848/duplicate found only the internal weekly-sweep digest — no update on the duplicate-transmission question or a Form 2848 confirmation. Google Drive: a title search for "Form 6166" (intended as a metadata check) was run **without** `excludeContentSnippets: true` and returned full content snippets of both certificates and the Form 8802 application — see §4 for what this did and did not settle, and the identity-block leak this caused (never written down; flagged in this run's report). Ping org-wide semantic search (several phrasings on 8802/2848/residency) returned no legible content specific to this client — same off-topic pattern as every prior run; `resolve_person` not re-tried (no change expected). `FOLLOW-UPS.md` row on this matter re-checked — still tracked there, nothing new; `BACKLOG.md` — no hits. **Chase pass (unbounded) on all outstanding items** — see below; no arrivals.
- 2026-09-19 — **Weekend sweep (incremental, baseline 2026-09-12→2026-09-19).** ✅ **Drive searches this run used `excludeContentSnippets: true` on every call, with no exception — the 2026-09-12 incident was not repeated.** Double: `list_client_properties` current; `list_notes` still 0; `list_activity_log` (from 2026-08-29, re-checked) returned zero entries. Gmail bounded `after:2026/09/12` for both names/"Form 6166"/2848/"Form 8802"/duplicate found only the internal weekly-sweep digest — no update on the duplicate-transmission question or a Form 2848 confirmation. Google Drive: a title search for "Ihor Naum" bounded to `modifiedTime > 2026-09-12`, run **with** `excludeContentSnippets: true`, returned **zero files** — no new or modified documents in the window, and **no content of any kind was returned or read**. Ping org-wide semantic search (several phrasings on 8802/2848/residency) returned no legible content specific to this client — same off-topic pattern as every prior run; `resolve_person` not re-tried. `FOLLOW-UPS.md` row on this matter re-checked — still tracked there, nothing new; `BACKLOG.md` — no hits. **Chase pass (unbounded) on all outstanding items** — see below; no arrivals.
- 2026-09-26 — **Weekend sweep (incremental, baseline 2026-09-19→2026-09-26).** ✅ **`excludeContentSnippets: true` used on every Drive search this run, no exception — no repeat of the 2026-09-12 incident.** Double: `list_client_properties` current (no changes); `list_notes` still returns **0 notes** — matches every prior sweep; `list_activity_log` bounded `fromTimestamp=2026-09-19` returned zero entries. Gmail bounded `after:2026/09/19` for both spellings ("Ihor Naum"/"Ihour Naum"/"Olha Levchuk") plus "Form 6166"/"Form 8802"/2848/duplicate found **2 threads**, neither client-related: an unrelated iKids Group insurance-quote email and the firm's own 2026-09-19 weekly-sweep digest — no update on the duplicate-transmission question or a Form 2848 confirmation. Ping: `resolve_person` still resolves to the single Ping client record found 2026-08-15 (no change); `search_meetings` scoped to that client ID (8802/2848/residency-certification phrasing) returned **zero results** — still no searchable meeting content for this client, consistent with every prior run. Google Drive: a title search for "Ihor Naum" bounded to `modifiedTime > 2026-09-19`, run **with** `excludeContentSnippets: true`, returned **zero files** — no new or modified documents, no content of any kind returned or read. `FOLLOW-UPS.md`/`BACKLOG.md` re-checked — no change. **Chase pass (unbounded) on all outstanding items** — none arrived; ages below.

### Outstanding items (CI-only — never in the SOP)

- ✅ **Watch for IRS mail — ARRIVED 2026-08-19, content CONFIRMED complete 2026-09-12.** Both Form 6166 certificates (2023, 2024) are in Drive and are now confirmed to be the genuine, complete IRS certification letters (§4) — closed.
- **Client was calling the IRS** (267-941-1000, "U.S. residency" option) — with the certificates now in hand and confirmed complete, this item is effectively moot; not chased further this run.
- Ask the client to raise the possible duplicate from the two 2026-05-06 transmissions — STILL OPEN (a client-side action, not a searchable arrival), ~143 days pending since the transmissions, no deadline. A targeted Gmail search bounded `after:2026/09/19` found nothing new.
- Confirm the **2023 and 2024 returns were on file** when the application was transmitted — STILL OPEN, not chased this run (budget; lower priority now the certificates have arrived and are confirmed complete).
- Confirm a **Form 2848 for Ihor Naum as an individual** is on file — STILL OPEN/UNCONFIRMED; a targeted Gmail search bounded `after:2026/09/19` found nothing relevant.
- Archive the filing evidence in the client's Drive folder as one PDF — not chased this run (budget; internal firm task, low priority with the certificates received and confirmed).
