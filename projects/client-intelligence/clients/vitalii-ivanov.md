# VITALII IVANOV & TETIANA MOGYLOVA

> **Status:** Active · **Owner:** Firm · **Last updated:** 2026-09-26

> ✅ **First full historical sweep completed 2026-08-29** (Ping org-wide — nothing found; Gmail full history; Drive; Double activity log — all unbounded). See §6 log.

> **Sensitive data lives in the firm's systems, not here.** This file holds
> non-sensitive knowledge and links only. Logins, passwords, full account numbers,
> dollar figures, and personal contact details stay in Google Drive / Double
> / QuickBooks and are referenced by link. Never paste a secret or personal data
> into this file.
> **A business EIN is the exception and MAY be written here** — it is public on Sunbiz,
> so hiding it protects nothing _(Lilian, 2026-08-12)_. An **SSN or ITIN never may**,
> including when it is the entity's tax ID. Write the EIN **hyphenated** (`12-3456789`) — nine
> bare digits trip the published-page gate and stop the build.

> **Two zones — what feeds the SOP vs what stays here.** This file is the master
> record. Its sections split into two zones:
> - **Operating (feeds the client SOP):** §1 Snapshot, §2 Contacts, §3 Systems &
>   access, §4 Obligations & recurring processes, §5 Key facts & quirks, §7 Links —
>   the standing info a covering bookkeeper needs to run this client.
> - **Working context (CI-only — never in the SOP):** §6 — the log and outstanding
>   tasks/meeting follow-ups. Live tasks live in Double / Ping (linked), not copied
>   here.
>
> The SOP is the curated view of the **Operating** zone. See the project README
> ("Client Intelligence ↔ the client SOP") for how the two stay in sync.

> ✅ **RESOLVED 2026-09-26 — the USPS mail-forwarding order did NOT lapse on its 2026-09-18
> expiration date.** Last week's file left this unconfirmed either way. This sweep found USPS
> Informed Delivery digests addressed to Tetiana arriving **every single day from 2026-09-19
> through 2026-09-25 inclusive** (seven consecutive days past the stated expiration), still landing
> in the firm's inbox. **No renewal action, lapse notice, or client conversation about the order was
> found in any source** — the digests are the only evidence, but they are direct and current.
> ⚠️ **Read this as "still forwarding as of yesterday," not as "renewed" or "permanent"** — nobody
> has established *why* it continued (an auto-renewal, a longer original term than assumed, or simply
> that USPS's stated end date was not exact). See §5 and §6.
> 🔴 **STILL OPEN, unchanged: the foreign-entity reporting question on the four Ukrainian companies**
> — no new evidence found this sweep despite a targeted, unbounded chase (§6). Now **~43 days** since
> first recorded (2026-08-14), and the 2025 return was filed 2026-09-01 with this question's
> resolution still unconfirmed.
> ✅ **The 2025 return remains FILED** (Double: 2026-09-01) — see §4/§6.

## 1. Snapshot

- **Business name:** Vitalii Ivanov & Tetiana Mogylova — an **individual (joint) client record**, not a company
- **Entity type:** Individual taxpayers — Form 1040 _(Double: `Account Type = Individual`, `Tax Return Type = 1040`, read 2026-08-14)_
- **Home state:** _(pending)_ — 🔵 **corroborating signal, not a confirmation** _(2026-08-29)_: Drive holds a document titled **"Vitalii Ivanov Residency Status"** (created Sept 2025, last touched Dec 2025) and a Visa-application document folder dated Oct 2025 — **not read this sweep** (title-only Drive search per the redaction discipline). Its existence, plus a July 2026 email in which he refers to "until my return to the US," suggests he may currently be **outside the US** — this bears directly on his filing status and should be opened before the 2025 review proceeds.
- **Industry / what they do:** **In the US he is the owner of Deep Tech Development Group LLC** — e-commerce (robotics), trading as **"Go Robots"** at **gorobots.us**. He is **also the Double admin portal contact on 1701 N M ST LLC and Universal Trading Technology LLC — which is NOT the same as owning them**, and nothing has established that he does (§5). Abroad he holds **50% of four Ukrainian companies** (§5). ✅ **This answers what the file called "US-side activity it cannot yet name":** the **robot-purchase invoices** Lilian sent him in July 2026 are inventory for that store _(Lilian, 2026-08-25 — she named him its "dueño verdadero"; corroborated by Double contacts and the store's own billing emails to Julia)_
- **Primary language:** 🔴 **Russian only** _(Double `Preferred language = Only Russian`)_ — write to this client in Russian, not English
- **Our engagement (services we provide):** individual income tax (1040). Bookkeeping **N/A**, `1099 Preparation = false`, `Annual Report = false` _(Double properties)_
- **Fiscal year-end:** calendar year
- **Accounting platform:** Double `platform: none` — **no QuickBooks connected** _(2026-08-14)_

## 2. Contacts

Names, emails, and phone numbers are **personal data** — they live in Double, not
here. This section records **who plays which role**; open the Double client to get
the actual details (and Claude can pull them live when a task needs them).

| Role | Where to find them |
|---|---|
| The clients — a married couple filing jointly | Double client (link below) |
| Assigned staff | **Lilian Gonzalez** _(Double)_ |
| Preparer on the 2025 return | **Irina Jandieri** _(Double tax project)_ — also the author of the 2025 note in §6 |

- **Double client:** [app.doublehq.com/close?cid=710666](https://app.doublehq.com/close?cid=710666)
- **Double case note:** none **on this individual record**. The live matter concerning him sits on his company, **Deep Tech Development Group LLC** — Double note **503544**, on [`deep-tech-development.md`](./deep-tech-development.md).

## 3. Systems & access

| System | What it's for | Where credentials live (Drive link) | Non-sensitive reference |
|---|---|---|---|
| Double client portal | Tax organizer + document exchange | n/a | 2025 organizer **Sent**, not completed (§4) |
| Google Drive | The foreign-entity working file | _(pending — Drive link)_ | `Form5471_ExtremePride_2025.xlsx` — one entity only (§5) |

## 4. Obligations & recurring processes

### Sales tax
- **Applies?** _(pending on this individual record — nothing recorded.)_ ⓘ **His US business activity is now named (§1) and it sells tangible goods**, but that obligation sits on the **company**, not on this 1040 record — see [`deep-tech-development.md`](./deep-tech-development.md) §4, where the sales-tax registration is itself still an open question.

### Payroll
- **Applies?** No _(Double)_

### Bookkeeping & monthly close
- **Applies?** **No — `Bookkeeping = N/A`** _(Double)_; no QuickBooks connected

### Income tax
- **Applies?** **Yes**
- **Return type(s) & deadlines:** **Form 1040**, calendar year, due April 15 _(Double `Tax Return Type = 1040`)_
- **Our role:** the firm prepares and files
- **Current status:** ✅ **2025 return FILED, 2026-09-01** _(Double activity log: "2025 Taxes" project moved Waiting on Client Approval → Filed, 2026-09-01, by Lilian Gonzalez; the return had moved In Progress → Ready for Review on 2026-08-31 by Irina Jandieri, then → Waiting on Client Approval the same day)_. A signed copy (`IVANOVandMOGYLOVAVITALIIandTETIANA2025.pdf`) was filed into the client's Drive "2025" folder 2026-09-01. A new Double property, **`Signature = Signed`**, corroborates. **This resolves the "unfiled" status this file carried as of 2026-08-29.**
- **Organizer status:** ⚠️ **Still reads `Sent` in Double as of 2026-09-12** _(unchanged property value)_ — **not updated to reflect the return now being filed.** Read this as a further instance of the Double hand-maintained columns lagging the real work, not as evidence the organizer was never actually completed. Not chased further (a Double-property staleness question, not a client-facing open item).
- **Process notes (→ future SOP):**
  - 🔴 **A foreign-entity reporting question is open — see §5.** Whichever form applies (5471, 8865 or 8858) it **attaches to the 1040**, so `Tax Return Type = 1040` is **not** in conflict with it and must never be read as evidence that nothing is owed.

### Licenses & other filings
- **Applies?** _(pending)_ — nothing recorded on the US side

## 5. Key facts & quirks

> ⚠️ **Order these by consequence — only the first FOUR are published.** Both the Knowledge
> Hub and the client-intelligence review dashboard render **only the first four top-level
> bullets** of this section (and of §6's "Outstanding items"); a fifth never appears on
> either. So put first whatever would cause the worst mistake if someone didn't know it —
> **not** the oldest, and **not** whatever was added last. **Adding a bullet is a decision
> about where it goes**; appending to the end means the team never sees it. The cap lives in
> `clientCard()` — see the [render README's parsing contract](../../../.claude/skills/client-intelligence/render/README.md).

- ✅ **THE USPS MAIL-FORWARDING ARRANGEMENT FOR THIS COUPLE AND THREE COMPANIES, STATED TO END 2026-09-18, DID NOT LAPSE — CONFIRMED 2026-09-26.** _(USPS "End of Mail Forwarding" notices, 2026-08-14; found 2026-08-29.)_ The firm's address is the forwarding destination for **Tetiana personally** and for **Universal Trading Technology LLC**, **Deep Tech Development Group LLC**, and **1701 N M ST LLC**. Daily USPS Informed Delivery digests addressed to Tetiana have now been confirmed arriving **every day through 2026-09-25** — seven days past the stated expiration — so mail is still reaching the firm. **Nobody has established why it continued** (auto-renewal, a longer actual term, or an imprecise stated end date) — treat as "still active as of yesterday," not as permanently resolved; recheck if digests ever stop.
- 🔴 **HE HOLDS 50% OF FOUR UKRAINIAN COMPANIES — A FOREIGN-ENTITY REPORTING QUESTION IS OPEN ON ALL FOUR, AND IT IS LIVE WORK.** ⚠️ **Which form is not settled, and the file must not pretend it is.** Two things stop it being automatic: **50% is not a majority** — Form 5471's control category needs *more than* 50%, and a CFC needs US shareholders to hold *more than* 50% of vote or value, so at exactly 50% with a non-US co-owner it may not apply at all (acquisition-year categories can still bite); and a Ukrainian **ТОВ is an LLC-type entity**, so for US purposes it may be a corporation (**5471**), a partnership (**8865**) or disregarded (**8858**) depending on classification. **A working file already exists in Drive (`Form5471_ExtremePride_2025.xlsx`), so somebody has already reached a view on one of them.** Establish the position for **each** of the four, and what has been filed for prior years — the penalty for a missed foreign-entity form is **per form, per year** and among the harshest in the code. ⓘ A **`form-5471-tax-analysis` skill exists** — it is a personal/synced skill, **not** in this repo's `.claude/skills/`, so a session that cannot see it should ask for it rather than improvise.
- ✅ **THE 2025 RETURN WAS FILED 2026-09-01 — no longer an open item.** Signed 2026-08-31/09-01 and moved to Filed the same window (§4/§6). The organizer property still reads `Sent` in Double, which is a stale hand-maintained column, not evidence the work was incomplete.
- 🔵 **HE IS THE OWNER BEHIND THE FIRM'S DEEP TECH WORK — and that company has a live, blocked handover.** **Deep Tech Development Group LLC** is his _(Lilian, 2026-08-25)_, and its **"Go Robots" store (`gorobots.us`) is registered under Julia and is being transferred to him** — **blocked** as at 2026-08-25, with the whole trail on the company's file and in Double note **503544**, not here. ⚠️ **On his other two US entities the evidence is weaker:** Double records him as the **admin portal contact** on **1701 N M ST LLC** and **Universal Trading Technology LLC**, and **a portal contact is not proof of ownership** — nobody has established who owns those. **His company matters belong in [`deep-tech-development.md`](./deep-tech-development.md), not in this file**; this bullet exists so nobody reads this record as though he had no US business. _(Double contacts, 2026-08-25.)_
- **Write to this client in RUSSIAN.** Double records `Preferred language = Only Russian`. It is the only client file so far carrying that property, and it means English correspondence will not land.
- **The four companies, with their registration dates** _(Irina Jandieri, 2025-08-18)_: **ТОВ Бест Вей Фудс** (2018-07-30) · **ТОВ Екстрим Прайд** (2019-06-05) · **ТОВ 34-й градус** (2020-07-04) · **ТОВ Юкрейн Анлімітед Трейдинг** (2020-11-17). The Drive working file names **Extreme Pride**, so that is the one already under analysis.
- ✅ **The firm's address is this couple's mail-of-record, formally, via a USPS change-of-address forwarding order** (see bullet 1 for the lapsing date) — not merely an informal habit. Digests continued daily through 2026-08-28; the firm has also forwarded her physical mail on to her directly (§6).

## 6. History & open questions
<!-- CI-only zone: this whole section stays in Client Intelligence and never goes into the SOP. -->

### Log

- 2026-08-14 — **File created**, then **corrected the same day.** The first version stated the Form 5471 position as settled and left the mail-of-record question open; an independent review caught the overreach and a wider Gmail pass answered the second. _(Worked by Lilian.)_
  - **2025-08-18 — the ownership note.** _(Irina Jandieri, migrated TaxDome notes.)_ He holds **50% of four Ukrainian companies**, with each company's registration date (§5).
  - **2026-07-13 — Lilian forwarded him the letters received at Julia's address** — the firm's own mail-of-record arrangement, in his own correspondence.
  - **2026-07-28 — Lilian sent him invoices for robot purchases** _(in Russian)_. The first sign of US-side activity on this file. ⓘ **ANSWERED 2026-08-25** — they are inventory for the **"Go Robots" store** (`gorobots.us`) run by his company **Deep Tech Development Group LLC**; see the 2026-08-25 entry below.
  - **Throughout July and August 2026 — USPS Informed Delivery digests** addressed to the spouse arrive at the firm's inbox daily.
- **Nothing further was found in the sources actually searched** — Double live, and Gmail from 2026-07 forward. **Ping, Drive and everything earlier have never been swept**, so this is a statement about the search, not about the world.
- 2026-08-25 — **Established that he is the owner behind the firm's Deep Tech / "Go Robots" work**, while recording
  the store-ownership transfer being worked for that company. Double's portal contacts put him as admin on
  **Deep Tech Development Group LLC**, **1701 N M ST LLC**, **Universal Trading Technology LLC** and this individual
  record; the store's own billing emails to Julia name it **Deep Tech Development / gorobots.us**. That closes the
  "robot purchases" question this file had carried since 2026-07-28 and names the US-side activity §1 could not.
  **That matter itself is recorded on the company** — [`deep-tech-development.md`](./deep-tech-development.md)
  and Double note **503544** — not duplicated here. ⚠️ **What it does NOT establish** is that he owns **1701 N M ST
  LLC** or **Universal Trading Technology LLC**: Lilian's statement covered **Deep Tech only**, and on the other two
  all we have is portal-contact status. _(Worked by Lilian.)_
  - ⓘ **A loose thread this turned up, and it is an INFERENCE, not a finding.** The external Ukrainian
    finance-team contact who holds portal access to Deep Tech and to two of his other US entities uses an
    email domain that matches one of his **four Ukrainian companies** (**ТОВ 34-й градус**). If that is the
    same organisation it links the Ukrainian finance team to a company inside the open foreign-entity
    question above — **but it rests on the domain name alone and nobody has checked it.** Worth one question
    when that work is picked up. _(Double contacts, 2026-08-25.)_
- 2026-08-29 — **First full historical sweep.** Findings, by source:
  - **Ping** (`resolve_person`, org-wide `search_meetings` for the individual's name, the four Ukrainian company names, and "Deep Tech"/"Go Robots"/"Extreme Pride", `list_action_items`, `get_client_details`): **zero recorded meetings, zero action items** — `recentMeetingCount: 0`. No Zoom/phone call for this client has ever been captured by Ping.
  - **Gmail** (full history, no date bound — the earlier catch-up had covered only 2026-07 → 08): confirmed the couple's mail-of-record arrangement is a **formal USPS change-of-address forwarding order** ending **2026-09-18**, covering Tetiana personally plus **Universal Trading Technology LLC**, **Deep Tech Development Group LLC**, and **1701 N M ST LLC** on one notice (§5). Also found the 34thgrade.com.ua domain directly **CC'd** on Deep Tech invoicing (2026-08-19) and on Lilian's June financial-statement email (2026-07-02) — this corroborates, though does not prove, the loose ТОВ 34-й градус thread above. Confirmed a Gusto payroll-cancellation request from the client (Aug 5, "no employees now, and probably won't have any until I return to the US") and a QuickBooks $ invoice addressed directly to Universal Trading Technology LLC (2026-08-21) — both belong to Deep Tech's own file, not duplicated in full here.
  - **Google Drive** (`excludeContentSnippets: true`): a "Vitalii Ivanov Residency Status" memo (Sept–Dec 2025) and a Visa-application document folder (Oct 2025) — **titles only, not opened** (§1); a Form 8821 and Form 2848 both dated 2025-10-14; a 2025 estimated-tax payment confirmation (Jan 2026); an **`Ivanov_2025_Form8938_FBAR_Workpaper`** and an **`Ivanov_2025_FBAR_8938_Account_Summary`**, both created **2026-08-20/21 — i.e. work on the foreign-entity/foreign-account reporting question is already IN PROGRESS**, contradicting nothing in §5 but meaningfully advancing it: this is no longer only an open question, it is being actively worked. A check payable in connection with "o2b Kids!" and Tetiana Mogylova was also found (Oct 2025) — an unidentified possible income source, not chased further this sweep (see Outstanding items).
  - **Double** (`list_activity_log`, full history — 27 entries): confirms the organizer was published 2026-06-29, the engagement letter and organizer tasks were marked done 2026-08-10, and the "Prepare tax return" task has been toggled between Done/In Progress several times as recently as 2026-08-12 — consistent with active, ongoing preparation work rather than a stalled file.
- 2026-09-12 — **Weekend sweep (incremental, baseline 2026-08-29).** 🔴 **Headline finding: the 2025 return was FILED 2026-09-01** (Double activity log: In Progress → Ready for Review 2026-08-31 by Irina Jandieri; → Waiting on Client Approval same day; → Filed 2026-09-01 by Lilian Gonzalez; the signed PDF was moved into the client's Drive "2025" folder 2026-09-01). A new Double property, `Signature = Signed`, corroborates; `Organizer Status` still reads `Sent` (a stale column — see §4). No new Double notes (0). Gmail (since 08-29): a USPS Informed Delivery digest addressed to Tetiana on 2026-09-02 confirms mail forwarding was still active then; three duplicate Google Drive share-request emails (2026-08-31) from `v.ivanov@bestwayfoods.com.ua` (his Ukrainian company's own domain) asking for access to the firm's "3-Docs for Visa application" folder — not previously seen, access status unconfirmed. Google Drive: the visa-documents folder itself was last modified 2026-08-31 (matching the share-request timing); a title search for "Ivanov" also surfaced a "W-8BEN Ivanov Yaroslav.pdf" (2026-09-01) — a **different first name**, not linked to this client, flagged only to prevent a future false match. A targeted, unbounded chase for further foreign-entity (5471/8865/8858/"Extreme Pride") work in Drive and Gmail found nothing dated after the 2026-08-20/21 FBAR/8938 workpapers — no evidence the classification question was resolved before the return was filed. Ping: org-wide semantic search for "Vitalii Ivanov foreign entity 5471 mail forwarding" returned no legible, client-specific results.
- 2026-09-26 — **Batch sweep (incremental, baseline 2026-09-19→2026-09-26) — the USPS deadline was this run's priority, and it is now RESOLVED.** Double: `list_notes` on this individual record returned **zero notes** (unchanged — the matter still lives on Deep Tech's note 503544); `list_client_properties` unchanged (`Organizer Status = Sent`, `Signature = Signed`, `Preferred language = Only Russian`, `Tax Return Type = 1040`); `list_activity_log` bounded `fromTimestamp=2026-09-19` returned **zero entries** — no Double-side movement since the return was filed. **USPS mail-forwarding:** a `from:informeddelivery.usps.com` Gmail search confirmed a Daily Digest addressed to Tetiana arrived on **every single day from 2026-09-19 through 2026-09-25 inclusive** (7 consecutive days past the stated 09-18 expiration) — the order has **not** lapsed. A separate search for "mail forwarding"/"change of address"/"forwarding order" correspondence since 2026-09-19 found **no renewal action and no lapse notice** — the digests remain the only evidence, but they are current and unambiguous. **Foreign-entity question (5471/8865/8858):** a targeted, unbounded Gmail search for "5471"/"8938"/"FBAR"/"Extreme Pride"/"Best Way Foods"/"34-й градус"/"Ukraine Unlimited" since 2026-09-19 found nothing — the two hits returned were an unrelated Delaware-formation receipt and an unrelated Wise support thread, both false-positive keyword collisions, not client correspondence. **No movement on this question since the 2026-08-20/21 FBAR/8938 workpapers.** Google Drive: a full-text search for "Ivanov"/"Mogylova" bounded to `modifiedTime > 2026-09-19` returned two files, both unrelated to this client (a shared business-formation intake spreadsheet and Lilian's own hours tracker — false-positive full-text matches, not client documents). Ping: org-wide semantic search for "Vitalii Ivanov mail forwarding USPS foreign entity 5471 Ukrainian companies" returned no legible, client-specific results — same non-result pattern as every prior run.
- 2026-09-19 — **Weekend sweep (incremental, baseline 2026-09-12→2026-09-19) — the USPS deadline check was this run's priority.** Double: `list_notes` on this individual record returned **zero notes** (confirmed again — the matter still lives entirely on Deep Tech's note 503544, not here); `list_client_properties` unchanged (`Organizer Status = Sent`, `Signature = Signed`, `Preferred language = Only Russian`); `list_activity_log` bounded `fromTimestamp=2026-09-12` returned zero entries — no Double-side movement since the return was filed. `list_contacts` re-confirms Vitalii Ivanov's own Double contact record is linked to **four** client IDs total (this individual record plus three company records) — consistent with the "admin contact on Deep Tech + two more US entities" already on file; **which of the three company IDs maps to which entity name was not re-derived this sweep** (not asserted here to avoid a wrong match). **USPS mail-forwarding deadline (2026-09-18):** a `from:informeddelivery.usps.com` Gmail search confirmed a Daily Digest addressed to Tetiana arrived on **every single day from 2026-09-13 through 2026-09-18 inclusive** (2026-09-13, -14, -15, -16, -17, -18) — so the order was still actively forwarding mail on the day it was due to expire. A search for a 2026-09-19 digest, and a separate search for "mail forwarding"/"change of address"/"forwarding order" correspondence since 2026-09-12, found **no renewal action, no lapse notice, and no client conversation about it** — the only non-USPS hit was an unrelated Magnum/Sunoma thread. **Status: genuinely unresolved, not lapsed-and-unnoticed** — the last confirmed evidence is that forwarding was live through 09-18; whether it continued into 09-19 is unknown from any source this session can reach. Foreign-entity question: a targeted, unbounded Gmail/Drive search for "5471"/"8938"/"FBAR"/"Extreme Pride"/"Best Way Foods" since 2026-09-12 found nothing new — no further work beyond the 2026-08-20/21 FBAR/8938 workpapers already on file. Google Drive: a title search for "Ivanov"/"Mogylova" bounded to `modifiedTime > 2026-09-12` returned zero files. Ping: org-wide semantic search for "Vitalii Ivanov mail forwarding USPS foreign entity 5471 Ukrainian companies" returned no legible, client-specific results — same non-result pattern as every prior run.

### Tax year 2025 — the review

- **Status:** ✅ **FILED 2026-09-01.** Signed 2026-08-31/09-01, moved Ready for Review → Waiting on Client Approval → Filed inside that same 48-hour window (Double activity log). Organizer property still reads `Sent` — a stale Double column, not evidence of incomplete work.
- **Preparer:** Irina Jandieri _(Double)_.
- **The open question that dominated the year, and whose resolution before filing is UNCONFIRMED:** which foreign-entity form, if any, each of the four Ukrainian companies requires (§5). FBAR/Form 8938 workpapers were started 2026-08-20/21, before this return was filed — but **no source found this sweep (Drive, Gmail) shows further work on the 5471/8865/8858 classification question between 2026-08-21 and the 2026-09-01 filing date.** Whether that question was actually settled before the return went out, or the return was filed with it still open, is not established — worth confirming directly with the preparer.

### Outstanding items (CI-only — never in the SOP)

- 🔴 **Settle the foreign-entity reporting position for each of the four companies**, for 2025 and prior years — **5471, 8865 or 8858 depending on classification**, and 50% may put some of them outside 5471 entirely. _(Open since at least 2026-08-14 — **~43 days as of 2026-09-26**. Re-chased this sweep — an unbounded Gmail search for 5471/8938/FBAR/"Extreme Pride"/"Best Way Foods"/"34-й градус"/"Ukraine Unlimited" found NO further work specific to these four companies since the 2026-08-20/21 FBAR/8938 workpapers — the 2025 return was filed 2026-09-01 with this question's resolution still unconfirmed. This remains the single most important open question on the file: find out from the preparer whether it was settled before filing or is still owed.)_
- [x] ~~The mail-forwarding order for this couple and three companies was due to EXPIRE 2026-09-18~~ — **RESOLVED 2026-09-26: it did NOT lapse.** USPS Informed Delivery digests addressed to Tetiana confirmed arriving on **every day from 09-19 through 09-25 inclusive** (7 days past the stated expiration) — mail is still reaching the firm. No renewal action or lapse notice was found in any source; the reason it continued is unknown (auto-renewal, longer actual term, or an imprecise stated date). **Recheck if digests ever stop arriving.**
- [x] ~~Confirm the 2025 Form 4868, then chase the organizer~~ — **OVERTAKEN 2026-09-01: the return itself was filed**, which moots the extension-confirmation question. The organizer property still reads `Sent` in Double (stale column, not a live gap — see §4).
- **Open the "Vitalii Ivanov Residency Status" Drive document and the Oct 2025 visa-application folder** before the review proceeds — his physical location/residency status may affect filing status and is referenced but unread this sweep (§1). **New this sweep (2026-09-12):** a Vitaliy Ivanov, from an email address on his Ukrainian company's own domain (`v.ivanov@bestwayfoods.com.ua`), sent three duplicate Google Drive share requests (2026-08-31) asking to access the firm's own "3-Docs for Visa application - Vitalii Ivanov" folder. Not established whether access was granted; worth a decision on whether/how to share his own visa documents with him directly.
- ~~Find out what the "robot purchases" are (2026-07-28)~~ — **answered 2026-08-25:** they are inventory for the **"Go Robots" store** (`gorobots.us`) run by **Deep Tech Development Group LLC**, which is his. Forwarding those vendor invoices to him is a standing part of that company's bookkeeping relationship.
- **Unconfirmed, flagged not asserted:** a Drive file titled "W-8BEN Ivanov Yaroslav.pdf" (created 2026-09-01) surfaced in a title search for "Ivanov" — the first name **Yaroslav** does not match this client (**Vitalii**), so this is NOT recorded as connected to him; noted only so a future session does not re-discover it and wrongly assume a link.

### Information still needed

- [ ] What they do in the US, their home state, and whether either has US business activity — **partly answered 2026-08-25:** **he** has US business activity (he owns **Deep Tech Development Group LLC**, and is the admin portal contact on two more US entities whose ownership is unestablished — §5). **Still open: their home state** — 🔵 a Residency Status memo and a visa-application folder exist in Drive (2025) and were not opened this sweep; open them next — and whether **Tetiana Mogylova** has any US activity of her own (a check found in Drive, "o2b Kids!" payable in connection with her name, Oct 2025, is unexplained).
- [ ] Which of the four Ukrainian companies are still active, and their current ownership percentages
- [ ] What foreign-entity forms have been filed to date, and for which years
- [x] Whether a 2025 extension exists — **very likely yes** (the roster-wide May 2026 "File Extension" task was assigned and marked Done for this client, per Double activity log, 2026-05-25); not independently confirmed by an actual 4868 document in Drive this sweep.
- [x] Everything from Gmail, Drive, Ping and prior years — **first full historical sweep run 2026-08-29; see log above.** Ping returned nothing (zero meetings/action items); Gmail and Drive both extended coverage well before July 2026 (Drive folders back to 2025-06; earlier company history is Deep Tech's, not this individual record's).

## 7. Links

- **Double client:** [app.doublehq.com/close?cid=710666](https://app.doublehq.com/close?cid=710666)
- **Double tax project (2025):** [tax-return?cid=710666&projectId=219350](https://app.doublehq.com/tax-return?cid=710666&projectId=219350)
- **Migrated TaxDome notes:** Drive `*QBO Clients and Individuals > VITALII IVANOV` (and the `*Dupplicated` copy) — read 2026-08-13, written up here 2026-08-14.
- **Google Drive folder (sensitive vault):** `VITALII IVANOV` (Julia's Drive, plus a second "VITALIY IVANOV" folder tree) — confirmed present 2026-08-29; holds `Form5471_ExtremePride_2025.xlsx`, the 2026-08-20/21 FBAR/8938 workpapers, POA forms (2025-10-14), the Residency Status memo and the visa-application folder (both unopened this sweep)
- **Related clients (owner group):** [`deep-tech-development.md`](./deep-tech-development.md) — **Deep Tech Development Group LLC**, his US e-commerce company (the "Go Robots" store), where his company-side matters are recorded. **1701 N M ST LLC** and **Universal Trading Technology LLC** — he is their admin portal contact, ownership unestablished — have no CI file yet.
- **Related SOPs:** _(pending)_
