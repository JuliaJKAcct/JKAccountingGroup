...
- **Signature property (Double):** reads **Signed** _(Double client properties, 2026-09-12 — a property not previously read for this client)_ — consistent with the 2026-08-31 signed-return activity in §5, though it does not by itself resolve the K-1 contradiction.
...
- ⚠️ **CONTRADICTION, unsettled — the return that §5's own K-1 blocker says can't be finished shows as FILED in Double, and it was RE-FILED with a real client e-signature on 2026-08-31.** `list_projects` (2026-08-29) had shown the "2025 Taxes" project with **status `filed`, `filedAt` 2026-07-03**, preparer Liudmyla Kazannik. **This changed materially on 2026-08-31**: the activity log shows the project toggled **Filed → Ready for Filing** (19:23 UTC) and then **Ready for Filing → Filed** again (19:42 UTC) the same day, `filedAt` now reading **2026-08-31T19:42:08Z**. In the same 30-minute window: an **"Organizer" was opened by the client (Denys Rubis) at 16:48 UTC**, and a signed PDF (`ONETWOSTRATEGIESINC2025.pdf`, attached to transaction `signedSignatureRequest-175134`) plus a `2025 Annual Report Payment Receipt.pdf` were filed into Drive at 19:09 UTC. **This is real, fresh evidence that the return was actually completed, signed by the client, and filed on 2026-08-31** — much stronger than the mere project-status flag this file flagged as unverified last week. It still directly conflicts with the very next bullet (the two missing K-1s and the unentered home-office deduction, from a 2026-04-13 client call, now 141 days old with no arrival recorded in any source searched). Two live possibilities, neither confirmed: the return was completed and filed without those K-1 items (on the extension, closing out incomplete or with the client re-confirming they aren't needed), or they arrived through a channel this sweep hasn't found. **Do not assume either — ask before treating the K-1 chase as moot or the "filed" status as unreliable.** _(Double `list_projects` + `list_activity_log`, 2026-08-29 and 2026-09-12.)_
...
- 🆕 **The primary contact (Denys Rubis, `den@onetoconsult.com`) formed a NEW Delaware entity, ACRETIC LLC, on 2026-09-23** — incorporation documents through Harvard Business Services, Inc. (order confirmation, signature request, and Julia forwarding the signed documents to the contact all same day). **Whether ACRETIC LLC is or will become a JK Accounting Group engagement, and how it relates to OneTwo Strategies (a personal venture of the same contact vs. a business extension), is NOT established** — flag for confirmation before assuming it belongs in scope or is unrelated. _(Gmail, 2026-09-23.)_
...
- 2026-09-26 — **Manual incremental sweep, baseline 2026-09-19→2026-09-26.** Double: `list_activity_log` this window shows only a routine September month-end-close status change (2026-09-21); `list_notes` still returns zero; `list_client_properties` unchanged (Organizer Status still Sent, Signature still Signed); `list_projects` unchanged (2025 Taxes still `filed`, `filedAt` 2026-08-31). Gmail: a targeted search for "OneTwo Strategies"/onetoconsult/"2242 Monroe"/"Porcupine Partnership", bounded after:2026/09/19, found routine Gusto payroll notices (payroll ran normally, no cancel/re-run this window) plus a **new, unrelated fact**: the primary contact formed a new Delaware entity, ACRETIC LLC, via Harvard Business Services on 2026-09-23 (now §5) — **no correspondence at all on the two missing K-1s, the home-office deduction, the salary/withholding change, or the reemployment-tax PDF.** Ping not re-queried this pass (same non-result pattern on every prior sweep for this client; budget). **Re-verified per instruction, not resolved: the contradiction between the return showing FILED (2026-08-31, client e-signature) and the two K-1s/home-office deduction never confirmed as arrived stands unchanged — no new evidence either way this window.**
...
- **Chase the two missing K-1s** (2242 Monroe LLC, Porcupine Partnership) / home-office deduction — STILL OPEN, **≈166 days** pending since 2026-04-13, no arrival found in a search bounded 2026-09-19→09-26. ⚠️ **The §5 contradiction is unresolved either way** — the return shows filed with a client e-signature (2026-08-31), but nothing found this sweep confirms the K-1s arrived or that the client re-confirmed they weren't needed. **Ask before assuming either way — this now needs a direct question to whoever prepared the return, not another search.**
- **Read the 2024 `ONETWO STRATEGIES REEMPLOYEMENT TAX.pdf`** — not independently re-checked this sweep (budget); still unopened as of the last confirmed check (2026-09-19).
- **Confirm whether the discussed salary increase / withholding change was implemented** — STILL OPEN, the 2026-09-26 targeted search again found only unrelated correspondence.
- **Confirm the Accountable Plan is actually posted to QuickBooks** — STILL OPEN; not independently re-checked this sweep (budget).
- 🆕 **Confirm what ACRETIC LLC is and whether it is in scope** — new this sweep (§5); not yet chased, just found.
