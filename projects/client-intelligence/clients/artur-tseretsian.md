...
- **The older child's SSN** — SS card was lost. On **2026-07-31** the client went to the SSA office and was told the replacement card would be **mailed by post** (allow ~1 month), so it was expected around **late August 2026**. As of **2026-09-19** we still don't have the number — a Gmail search bounded `after:2026/09/12` for name/"Social Security" found nothing, and Double notes carry no update (~48 days pending). The client sends such items by **WhatsApp** (outside the digital sweep). **Next step:** confirm with the client that the card arrived and get the number — entered into Double, never the repo.
...
- **Rewhip LLC — clarify scope** — STILL OPEN, ~75 days pending since raised (2026-07-15), no deadline. A targeted search bounded `after:2026/09/12` found nothing beyond what's already on file.
- **Stripe transaction data** — STILL OPEN, ~84 days pending since raised (2026-07-06), no deadline. A targeted search bounded `after:2026/09/12` found nothing client-specific.
