...
> prompt first and paste them back in. 🛑 **Claude cannot do the write on THIS Routine** — tested
> 2026-08-24: `update_trigger` refuses an `http_api` Routine outright. What a session *can* do, and
> should, is hand you the finished block **with both real values already substituted in**, read out
> of the live prompt by `list_triggers` — so the paste is one copy and one paste, with nothing to
> carry across by hand. _(This line has now been wrong twice: once claiming the secret was
> unreadable, once claiming the write was possible. The tested statement is: **read yes, write
> no.**)_
...
2. COVERAGE CHECK — do this BEFORE sweeping, every run. It is what stops a client being silently missed, and it runs in THREE directions, not one. Nobody is swept because someone remembered them; they are swept because a reconciliation found them.
   ...
   ⓘ Baseline measurement, 2026-08-24 (reconciled by hand after the 2026-08-22 run): 49 client files = 34 ledger rows + 11 in the full-pass queue + 3 in the exclusion table + Kompozit USA, and all 49 are named in one of the two tables — so on that date this check found nothing. 🛑 THAT IS A DATED OBSERVATION, NOT THE EXPECTED ANSWER. This prompt is re-pasted by hand and rarely, so the figures go stale while the check does not: COMPUTE the counts every run and report what you actually find. A hit is a real finding, never a sign that you measured wrong.
   ...
   2c. THE FULL-PASS QUEUE → actually drains. 🔴 THE CLIENTS OWED AN EXPENSIVE FULL HISTORICAL PASS ARE TWO POPULATIONS, AND A RUN THAT COMPUTES ONLY THE FIRST STRANDS THE SECOND FOREVER. (i) IN SCOPE (2a/2b) WITH NO LEDGER ROW — 11 as at 2026-08-24, down from ~17, against a cap of ~6 per run: the ordinary backlog, roughly two runs deep. ⚠️ A PROSPECT IS NOT IN POPULATION (i) AND IS NOT IN THAT 11. A scope-table row whose Double id reads "(none — PROSPECT…)" has no Double, no Ping and no tax planes; its entire history is a handful of emails that cost nothing to re-read. Give it the cheap bounded Gmail + Drive pass EVERY run, do NOT spend a cap slot on it, and do NOT write it a ledger row — a row would bound searches that were never expensive. Kompozit USA is the current one. 🛑 M5 Studio Miami is NOT a prospect: its id reads "no Double account confirmed", which is an unfinished search, not a settled fact — it stays in the queue until someone searches Double by name and settles it. (ii) IN SCOPE, WITH A LEDGER ROW, whose Coverage-gaps cell carries "⚠️ CATCH-UP OWED" — a client whose full pass ran but finished INCOMPLETE. Population (ii) is invisible to a no-row test AND its baseline has already advanced, so every future search is bounded past the history nobody read and nothing else will ever bring it back. MERGE BOTH INTO ONE QUEUE and order them together. Order it yourself, deterministically: FIRST any client whose Double record shows platform: qbo or a Bookkeeping cadence (active, fee-earning work), THEN the rest; and WITHIN each group, OLDEST CLIENT FILE FIRST. ⚠️ Do not rely on the "Catch-up priority" block in weekend-ci-sweep.md for the head of the queue — its named first entries (Artur Tseretsian, Ihor Naum & Olha Levchuk) were swept on 2026-08-15 and now hold rows, and its stated depths ("ten deep", "~16") predate this count. Compute the queue from the ledger, not from prose — file age is something you can actually compute, unlike a deferral count, which is recorded nowhere durable. ⚠️ THE COVERAGE-GAPS COLUMN RECORDS WHICH SOURCE IS OWED — that is precisely what it is for, and a "⚠️ CATCH-UP OWED: <source>" note IS that, which is why 2c reads it. 🛑 WHAT MUST NEVER GO IN IT IS A STATUS MARKER — a deferral count, a "deferred twice", a "checked, nothing found" — because those describe what a RUN did rather than what the CLIENT'S data still lacks, and a run that reads them sends its budget at work already done. The test is one question: does the cell name a SOURCE that is still unread? Then it is coverage and it queues. Does it narrate a previous run? Then it does not belong in that column at all. So: do NOT try to derive a deferral count from this column — but DO read it for CATCH-UP OWED. A queue that is only ever appended to becomes a permanent exclusion list — Artur Tseretsian was deferred twice before being cleared — so if a client is STILL unswept after three runs, name them in the email.
   ...
   ✅ THE LEDGER ROW IS YOURS TO WRITE, AND WRITING IT IS WHAT STOPS THE LOOP. sweep-state.md IS inside your merge scope. Whenever you COMPLETE a first-time full historical pass — a client found here, or one off the 2c queue — ADD that client's row to sweep-state.md dated today, in the same commit. Skip it and next Saturday re-buys the identical expensive pass, forever. Write the row only for a pass you actually finished, and there are THREE outcomes, not two: (1) finished in full → write the row, plain. (2) never started, or the cap/context ran out → leave the row ABSENT and name the client in the email. 🔴 (3) THE PASS RAN END TO END BUT A SOURCE WAS ONLY PARTLY READ — a Gmail result set you did not page through, a connector that failed, a folder you could not open → WRITE THE ROW (the work was real) AND write its Coverage-gaps cell as "⚠️ CATCH-UP OWED: <source> — <what was not read>". Do NOT write it as a plain "first full historical sweep completed": the row advances the baseline, so a description of the gap in prose changes nothing while the words "completed" send step 2c straight past it. This is the 2026-08-22 defect exactly — VOICECAPITAL INC and VOXAGO LLC both got "completed" rows admitting, in the same sentence, that only the first page of their Gmail had been read. Never derive scope from one staff member's client list: doing that is how seven QuickBooks-connected companies went with no Client Intelligence at all until 2026-08-11.
...
4. Bound every search to that client's baseline date in sweep-state.md AND LATER, inclusive of the baseline day itself (items can land later the same day a sweep ran; duplicates are prevented because you read the client file first). Gmail: after:YYYY/MM/DD. Ping/Double: date >= baseline. Never re-read anything from BEFORE the baseline. 🔴 ONE EXEMPTION, AND IT IS THE ENTIRE POINT OF A CATCH-UP: when the client you are working is a step 2c population (ii) client — a ledger row carrying "⚠️ CATCH-UP OWED: <source>" — THE NAMED SOURCE IS SEARCHED UNBOUNDED, exactly as a first pass would search it. The baseline still bounds every OTHER source for that client. Without this exemption the catch-up is impossible: a bounded re-search of a source whose gap lies BEFORE the baseline returns nothing, and the run would then clear the note as satisfied while the history stays unread — the same false "completed" one level down. Clear the note ONLY when the named source has been read to the end; if the budget runs out again, REWRITE the note with what is still unread. Never clear it on a bounded search. EVERY client in scope gets its cheap incremental pass EVERY run — the cap applies only to the expensive first-time / coverage-gap FULL historical passes, at most ~6 per run, taken in the order step 2c computes — never a deferral count, which is recorded nowhere. If the cap makes you defer a client's full pass, NAME that client in the email so the queue stays visible. ✅ AND WHEN YOU COMPLETE A PASS THAT A "Coverage gaps" NOTE ASKED FOR, CLEAR THAT NOTE in the same commit — an un-cleared note re-prioritises a client for an expensive pass they have already had, ahead of clients who have never been swept. ⚠️ A "⚠️ CATCH-UP OWED" note is exactly such a note: it is QUEUE WORK under step 2c population (ii), it counts against this cap like any other full pass, and it is cleared only when the named source has actually been read to the end.
...
5. For each client, sweep the connected sources for what is NEW, searching by BOTH the business name AND each owner/principal name (a meeting titled with a person's name often covers the business). Owners with several businesses: sweep at the OWNER level across all their entities, then ROUTE each fact to the specific company file it belongs to — a Double INDIVIDUAL profile is that owner's 1040 work, the COMPANY record is sales tax / the company return / 1099s; never let one company's file absorb another's facts.
   ...
   - Double: get_client; list_client_properties (the cleanest structured input for the Operating zone — Assigned Staff, Entity / Tax Return Type, Sales Tax, Bookkeeping, Payroll, 1099 Preparation, Annual Report, Organizer Status); list_notes; list_contacts (ROLES only); list_activity_log. 🛑 NOTE EDITS ARE INVISIBLE TO list_activity_log — it records no Note entity at all, so a note whose CONTENT changed looks untouched, and the firm's own convention is that a working note is rewritten IN PLACE rather than replaced. list_notes returns every note's updatedAt AND its body in the same call (VERIFIED live 2026-08-20 on client 764785 — both notes came back with body and updatedAt populated), so: compare updatedAt against the baseline for EVERY note returned, and RE-READ the body of any note that moved. Do not stop at noticing that a note was touched — read what changed. A client whose notes you did not check this way has NOT been established as "no movement"; say what you checked, not that nothing happened. (This is why the 2026-08-15 run reported "nothing has moved" on Denys Melnyk while his remaining business expenses had arrived 2026-08-13 into note 485225 — found five days later by a human reading the note.)
...
6. CHASE THIS CLIENT'S OWN OPEN ITEMS — a SECOND, SEPARATE pass over the same client, and skipping it is the defect found on 2026-08-18. Step 5 asks the sources "what is new?". That question is structurally blind to three things: something that was supposed to happen and did not, a deadline approaching, and — worst — something that DID happen that nobody searched for, because only the client file knew to watch for it.
   So: OPEN clients/<slug>.md, read its "Outstanding items" and "Information still needed", and take each entry BACK TO THE SOURCES AS A NAMED QUERY — search for that specific document, receipt, licence or reply by name. Not a general "what changed" pass; a query for that one thing. Then for each entry report:
     ...
     - HOW OLD is it — in DAYS, with the date it has been pending since. "Awaiting X" with no clock is invisible; "Awaiting X — 19 days" is not. ⚠️ IF THE CLIENT FILE RECORDS NO START DATE, SAY THAT — "pending since unknown — no start date in the file" — never a bare "no update". An item you could not clock and an item you never chased print identically otherwise, which is the ambiguity this whole step exists to remove.
...
- STILL OPEN (from step 6) goes in the template's section 3 "Still needed", and EVERY entry carries its AGE IN DAYS and any deadline — or, where the client file records no start date, the words "pending since unknown — no start date in the file". Never a bare "no update".
- ✅ AND PUT THE STEP-6 COUNTS IN THE EMAIL, not only in your final message: "chased N open items across M clients; K items had no budget and are named below". 🛑 If K is zero, SAY "0 unchased" — silence there is unreadable, and a reviewer working from the email (which is how this report is checked) cannot otherwise tell a complete chase pass from one that never reported its gaps. Name every unchased item beside its client.
- 🔴 TWO RULES ABOUT "NOTHING NEW", BOTH BROKEN ON 2026-08-15:
  ...
  (b) 🔴 IN AT A GLANCE, THE DOT IS DECIDED BY THE WORST OPEN ITEM — never by whether something new happened, and never by proposals. Proposals and new-fact counts belong in the STATUS TEXT; ranking them above the open items is exactly what produced the OPTIC GOLD line.
      - RED #B23A3A — a deadline within 14 days, OR an open item 90 days or older. This is the row that must catch the eye.
      - AMBER #B8841C — any other open item.
      - BLUE #2F5E8C — nothing open, but new facts were saved.
      - GREEN #2F8F5E — nothing open and nothing new.
    EVERY row with an open item carries THE OLDEST ITEM'S AGE in its status text whatever its colour, and the status text carries everything at once: "2 SOP proposals · 1 new fact · oldest open 159 days". ORDER THE SECTION red first, then amber by age DESCENDING, then the rest.
    ⚠️ THE THRESHOLD AND THE ORDERING ARE LOAD-BEARING, not decoration. A flat "anything open = amber" defeats itself: on 2026-08-22 ALL 34 swept clients had something in section 3, so that rule would have printed 34 identical dots and discriminated nothing — worse than the thing it replaced. The severity split is what makes the eye land on the 159-day item.
    ⚠️ And the defect being fixed: amber used to mean "nothing new BUT items open", so the ranking (proposals ▸ new facts ▸ quiet) overrode it, and OPTIC GOLD INC printed a neutral blue "1 new fact(s) saved" while carrying a 15-day unopened Sunbiz notice AND a 159-day IRS-address item, with no age at all. A one-line summary must answer "what is rotting?", not "did anything happen?".
