...
| GOSSIP MIAMI LLC | [`clients/gossip-miami.md`](./clients/gossip-miami.md) | Lilian | In progress — tax-prep only (business return + the owners' individual return + a Schedule C); financial-statement cleanup quoted separately and under way, **the business has been SOLD** (nothing about the sale documented). The client answered the 2026-08-11 cleanup questions: **checking account closed, company card now personal, the recurring checks are a contractor** → a **2025 1099-NEC is owed and there is no W-9**. Still waiting on three transactions, the closing statements and the check images (live status in §6) |
