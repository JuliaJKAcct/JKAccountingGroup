...
| [Tax Returns](./projects/tax-returns/) | The **working papers** behind each return the firm prepares — every line, every figure, where it came from and why each choice was made, so the reasoning outlives the session that produced it. The **only** place in the repo that holds client dollar figures, under a strict no-identifiers rule. Not published to the Knowledge Hub. | Active |
...
| [Acquisition Diligence](./projects/acquisition-diligence/) | Financial **due-diligence** work for clients buying a business — the industry yardsticks a target is measured against: cost structure, the KPI set, the three revenue-reasonableness tests (incl. a live water-method calculator), county utility inputs and valuation multiples, every figure graded for how far it can be pushed. | Active |
