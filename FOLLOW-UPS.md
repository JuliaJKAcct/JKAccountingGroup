...
| 10 | **Tax-season readiness list** — bookkeeping clients ready vs pending (for Julia) | Lilian | Double "Tax Returns – View 2" · the [`tax-season-readiness`](./.claude/skills/tax-season-readiness/) skill | Analysis done for the QuickBooks/bookkeeping companies **and** their owners. Blocked on three inputs from Lilian: (a) the **`Organizer Progress` %** for everyone at `Organizer Status = Sent` — the Double MCP cannot read that column; (b) confirming the **5 companies whose `Organizer Status` is blank** (no completed TaxDome organizer found in their folders); (c) resolving **2 status↔filed-date contradictions** and **3 missing/mismatched tax projects**. Also pending a decision: does "bookkeeping client" mean QBO-connected or the `Bookkeeping` property (they disagree). Then **group 3** — the tax-only clients — hasn't been started | 2026-07-30 |

| 11 | **Individual (1040) organizer question bank** — capture the firm's actual questions | Lilian | [`.claude/skills/tax-season-readiness/references/individual-organizer-questions.md`](./.claude/skills/tax-season-readiness/references/individual-organizer-questions.md) | Lilian asked for the organizer questions to live in the repo because the **Double MCP cannot read the organizer template** (confirmed 2026-07-30: no organizer tool; `get_task_templates` returns only task templates). The file is built with the verified scaffolding but the **question bank is deliberately empty** — it must not be filled from general knowledge. Next action: Lilian supplies the questions (paste / screenshot the organizer / a **blank** template PDF / dictate), then Claude structures them into the tables, reads them back for confirmation, and commits | 2026-07-30 |
