...
| 6 | **Monthly/quarterly retainer proposal** — productize Julia's proposal model | Lilian | [`projects/proposal-tool/`](./projects/proposal-tool/) — engine `generator-scripts/gen_premium.js` + `premium_proposal_body.js`; template `templates/JKA_Monthly_Proposal_Engagement_TEMPLATE.docx/.pdf` | Julia built the premium monthly-retainer proposal (used for Atman: one bundled fee, bilingual RU/EN, visible e-sign, T&C auto-renew + 5%/yr). **Built** (first English version) — interactive `tools/monthly-proposal-generator` fed by the shared pricing core, faithful to the 10-page premium model, editable, Save PDF; embedded in the Hub. Remaining: the **bilingual RU+EN** option (Cyrillic fonts, Atman-style) and the full-bleed closing-page polish; then add to the `proposal-generator` skill | 2026-07-28 |
...
| 8 | **Knowledge Hub** — publish the live link | Lilian | [`projects/knowledge-hub/`](./projects/knowledge-hub/) | All three interactive tools are now embedded in the Hub (Templates view + in-Hub readers): the business engagement-letter generator, the **monthly proposal generator**, and the **internal pricing calculator**. Remaining, pending **Lilian's go-ahead**: **publish the Hub to its official link** so the team sees the latest | 2026-07-28 |
