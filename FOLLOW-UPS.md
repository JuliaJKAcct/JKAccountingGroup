...
| 9 | **Andrii Matiukha — duplicate individual accounts in Double** | Lilian | Double — two individual clients for the same person (one name has a typo), both `1040-NR`, both tax status In Progress | Merge/remove the duplicate so one individual account carries the 1040. Surfaced 2026-07-30 while building the tax-readiness list; Lilian is fixing it the next working day | 2026-07-30 |
| 10 | **Tax-season readiness list** — bookkeeping clients ready vs pending (for Julia) | Lilian | Double "Tax Returns – View 2" · the [`tax-season-readiness`](./.claude/skills/tax-season-readiness/) skill | **Groups 1 + 2 delivered 2026-07-30** as an artifact for Julia (QuickBooks bookkeeping companies + their owners, four buckets each), built from Lilian's CSV export of the view — which unblocked the organizer percentages. Remaining: (a) Julia reviews the report; (b) **3 QBO companies are absent from the view** because they have **no 2025 tax project** (Atman Parts, Beemold USA, Aura Remodeling) — create the projects or confirm they're out of scope; (c) confirm **Ecoorganic USA**'s blank `Organizer Status`; (d) **Mobilesource Corp** shows `Sent` with no active organizer and no % — check whether the organizer was withdrawn; (e) decide whether "bookkeeping client" means QBO-connected or the `Bookkeeping` property (they disagree — 5 live clients differ). Then **group 3**, the tax-only clients, hasn't been started | 2026-07-30 |

| 11 | **Individual (1040) organizer question bank** — capture the firm's actual questions | Lilian | [`.claude/skills/tax-season-readiness/references/individual-organizer-questions.md`](./.claude/skills/tax-season-readiness/references/individual-organizer-questions.md) | Lilian asked for the organizer questions to live in the repo because the **Double MCP cannot read the organizer template** (confirmed 2026-07-30: no organizer tool; `get_task_templates` returns only task templates). **Partially captured 2026-07-30** from a 2023 organizer a client had filled to ~46% — General Information and Income (partly) are recorded verbatim, plus the two Google Sheets templates it sends clients to. Next action: supply a **fuller** organizer (ideally a blank template, or one near 100%) so the unseen sections can be added — document-upload pages, deductions & credits, health/HSA, education, retirement, estimated payments, refund banking, state questions, signature. Also worth checking: the dependents question hardcodes a specific date ("born AFTER December 31, 2024") — confirm it's rolled forward each season | 2026-07-30 |
