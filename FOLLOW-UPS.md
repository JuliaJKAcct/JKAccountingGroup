...
| 10 | **Tax-season readiness list** — bookkeeping clients ready vs pending (for Julia) | Lilian | Double "Tax Returns – View 2" · the [`tax-season-readiness`](./.claude/skills/tax-season-readiness/) skill | **Groups 1 + 2 delivered 2026-07-30** as an artifact for Julia (QuickBooks bookkeeping companies + their owners, four buckets each), built from Lilian's CSV export of the view — which unblocked the organizer percentages. The per-client detail lives in that artifact and in Double, not here. Open items are listed on the artifact's "needs a decision" panel: a few companies have no 2025 tax project and so are invisible in the tax view; one organizer state is unconfirmed; one client shows an organizer contradiction; and the firm still needs to decide whether "bookkeeping client" means QuickBooks-connected or the `Bookkeeping` property (they disagree). Then **group 3**, the tax-only clients, hasn't been started | 2026-07-30 |
