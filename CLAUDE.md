...
│       ├── hud-acquisition-allocation/  a client entity BOUGHT a property — split the price land vs building, capitalise the right
│       │                              closing costs, and prove every figure back to the settlement statement
...
| 🏠 **A client entity BOUGHT A PROPERTY** — a **HUD-1 / ALTA settlement statement**, a closing disclosure or a deed arrives and someone asks for "the HUD tool", an **acquisition allocation**, a **land/building split**, a **depreciable basis**, or the **opening journal entry** | the [`hud-acquisition-allocation` skill](./.claude/skills/hud-acquisition-allocation/) — drives the Bodner & Clark *HUD Tool v3 (Investor)* workbook the firm's property clients use. It carries the **four-bucket model** — facilitative costs → **building basis**, loan costs → **amortised over the loan**, operating costs → **expensed now**, escrow → a **current asset** — and the fifth block that is **not a cost at all**: *Reductions to Amount Due* is how the deal was **FUNDED** (earnest money, loan proceeds, seller credits), and miscoding one of those as a cost is the commonest way to break the workbook. 🔴 **Two traps it exists to stop.** ① *Taxes UNPAID by seller* (HUD 210–212) is a **credit and a liability** — the buyer pays the county later; *taxes PAID IN ADVANCE by seller* (HUD 106–108) is a **real cost this year**. They sit two inches apart and coding them alike overstates the deduction. ② **The assessor card is fetched for two numbers, so two numbers get read** — on the pilot the rest of that card said the entity had **already SOLD the property**, to a named buyer, eleven months on; nothing in the closing package hinted at it. **Read the whole card**, and check **which YEAR** its assessment is, because the defensible split is the ratio for the **year of acquisition**. ⚖️ **The proof is three reconciliations that must land on exactly $0.00** against settlement-statement lines **120 / 220 / 303** — a non-zero is a charge in the wrong bucket, never a rounding difference, and nothing ships until they are zero. *(They tie **independently of the land/building split**, so clear them while the assessor card is still being chased.)* 🛑 **And the gate the workbook CANNOT apply itself: was the property ever PLACED IN SERVICE?** Depreciation starts when it is ready **and available** for its intended use — not at closing, and a renovation can push that into a later year. **If the entity bought, renovated and sold without ever renting it, there is no depreciation at all** and the property may be **inventory with ORDINARY gain** rather than a §1231 asset. The tool computes a 27.5-year schedule for a flip without complaint. ⛔ **A session establishes the facts and puts the question — it never decides this.** ⚠️ **One environment fact:** county assessor sites are **blocked by the network egress policy** on the firm's cloud environments, so the property card comes from a person, as a PDF or a screenshot. Never invent a split and never fall back on "about 20% land". Read-only throughout; the filled workbook is delivered to the user and **never committed**, and the client-intelligence file gets the facts without the figures |
