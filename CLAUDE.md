...
│   ├── tax-returns/               THE WORKING PAPERS behind each return we prepare — every line, every
│   │                              figure, every decision; figures live HERE, never in client-intelligence (Active)
...
│   ├── export-chat/  save a session's CONVERSATION to a .txt before the session is
│   │              deleted — the chat only, never tool calls or internal reasoning;
│   │              a session can only export ITSELF
...
│       ├── personal-card-reimbursement/  an owner's PERSONAL card → what the company reimburses; the purpose gate
...
│       ├── organizer-review/          the PRE-RETURN REVIEW COMPANION (Lilian's "tax preparer") — reads all TEN sources on a client before their return — vs. the prior year, the notes and the files; outputs the grouped question list for the client
...
│       ├── tax-return-sop/            SOPs for TAX RETURNS — one per form; why they differ, the spine,
│       │                              the delivery format for a live return, and pull-the-PDF
...
| **What we actually put on a return we already prepared** — which figure went on which line, where it came from, why a choice was made, or what carries into next year | [`projects/tax-returns/`](./projects/tax-returns/) — one working paper per return, and **the only place in the repo that holds client dollar figures.** It exists because sessions get deleted and the reasoning behind a return is worth more than the finished PDF. ⛔ Its README carries the hard line: **never** an SSN/ITIN, bank or card number, home address, date of birth or login. **Not rendered by the Knowledge Hub, ever** — ⚠️ *but the ONE briefing rendered from a working paper and handed to the reviewer IS team-facing; handed over, never hosted.* 🔗 **A return coming BACK to be reviewed** — someone *(in practice Julia)* brings it back and starts asking: the financial statements, **or** the return, **or** just a question about a line; **any ONE of those is the trigger** — is [`tax-return-sop`](./.claude/skills/tax-return-sop/) **§4C**: **brief her from the working paper before opening the PDF**, never audit her. 🔒 **And opening that PDF carries §4A's rules — through [`tools/redact-doc/`](./tools/redact-doc/), say which document and why, never from a subagent, never from a scheduled session** |
...
| 🔴 **PREPARING A CLIENT'S TAX RETURN** — *"prepárame el Tax Return de X cliente"*, *"prepare X's return"*, *"hazme la declaración de X"* — or **writing/extending a TAX-RETURN SOP** for a form we do not have one for yet (1120, 1065, 1040, 1041, a state form) | the [`tax-return-sop` skill](./.claude/skills/tax-return-sop/) — one SOP **per form**, written for someone who has never filled that form in, so it says *where each number comes from* rather than which box it goes in. Carries the method (**reproduce last year's filed return from last year's books** — every difference is a convention nobody wrote down), the required spine, the delivery format for a live return (a table per form — 🗺️ **addressed by form · page · part · line AND, on a grid form, the COLUMN**, because *"Schedule E page 2 Part II"* is a page with two places to write; ⌨️ **and every row marked TYPED or COMPUTED**, since at the keyboard the first question is not *what is the number* but *do I type this one* — nine of Form 7203 Part I's fifteen lines are outputs — **with the FORM's answer and the SOFTWARE's given separately** — the order of preparation, the flow of figures between forms, the checkboxes with their reasons, the explanations, **the statements and attachments the return requires — drafted, because some of them block e-file** — **every K-1 read box by box, both the issuing and the receiving half**, and 🛠️ **the ENTRY ROUTE — where each figure is actually TYPED in the software**, because most lines on a computed form cannot be typed where they appear and a blank one means a missing input upstream, **and a WRONG one means a wrong input, which the software can never flag** — and 🛑 **every finding delivered WITH ITS FIX**: where, **what to type**, how to know it worked, and what it moves, because *"necesito que me digas cómo corregirlo, no que simplemente lo señales"*, and 🔗 **ONE RETURN PER REQUEST** — *"si te digo que preparo el tax return de una compañía, no preparo el del dueño"* — the company's work ends in a **handoff** (§8 of its working paper: the tables the owner's 1040 needs, left ready), and **the 1040 is a separate request on a separate day**), all pitched at **someone who knows nothing about taxes or forms**, and 🛑 the rule that any answer which changes a figure is read off the **current-year PDF from irs.gov**, never from memory — the IRS renumbers and inverts questions, and this firm has been caught by both. 📄 **A return analysis is delivered AS AN ARTIFACT by default** *(Lilian, 2026-09-06 — she types into ATX from the page)*, and the page carries **more** than the chat did, never less *(one declared carve-out, below)*: every figure with **its form, page, part and line**, **the arithmetic that produced it**, **where it is actually typed** *(and whether that route is established or guessed)*, and **any text destined for the return in ENGLISH, ready to paste** — [`tax-return-sop`](./.claude/skills/tax-return-sop/) §4B items 11-12. ☑️ **AND A LIST OF CHANGES SHIPS AS CHECKBOXES** *(Lilian, 2026-09-06 — item 13)*: she works down it at the keyboard and has to see **what is left**, not re-read it to find her place. **A real tickable box that REMEMBERS itself** on a page, `- [ ]` in chat; the **box first** on the row; a **running count**; grouped **by where she is working**, never by importance; and ⛔ **only on things she can DO** — a box beside information is what makes the column stop working. ⚠️ **The ticks live in her browser alone — never treat one as evidence something was done; the working paper is the record.** ⛔ **PHASE 2 ONLY — phase 1's REVIEW output is never an artifact** *(an artifact is a URL that **travels onward with no further act by the firm**, which is what makes the delete-the-session control real; the sanctioned container for the review is a **PDF**, and in a two-phase run it is **deferred to the end of the whole return** and covers the review AND the tables — [`organizer-review`](./.claude/skills/organizer-review/) §0 rule 4 and §5 step 4)*; when §4A sends both in one reply, **the review goes in the chat and the tables go on the page.** ⚠️ **And a SECOND limit is not settled: [`double-mcp`](./.claude/skills/double-mcp/) §2.2 point 1 bars ORGANIZER DATA from any artifact without narrowing itself to identifiers, while phase 2 reads the organizer as a required source.** ⛔ **Until Lilian rules, an organizer-sourced value — read from `get_organizer_responses` or from a TaxDome-era organizer file, and not independently established — stays OFF the page and goes in the chat AND the end-of-return PDF, with the page naming which rows are held back and where their values are.** 🔑 **That is a question to put, never to reason around** — both readings and the evidence for each are in [`FOLLOW-UPS.md`](./FOLLOW-UPS.md) row 87. Every prepared return leaves a working paper in [`projects/tax-returns/`](./projects/tax-returns/). Builds on [`sop-authoring`](./.claude/skills/sop-authoring/). 🔑 **§4A is the ENTRY POINT, and one sentence runs TWO PHASES** _(Lilian, 2026-08-20)_: 🔍 **Fase 1 · la REVISIÓN** — the [`organizer-review` skill](./.claude/skills/organizer-review/) **in full, ALWAYS, without being asked separately** (does the prior year raise an alarm? what documents are missing? what contradicts what?), whose **Block A verdict is ⚖️ THE GATE**: *yes* → straight on; *no* → **stop at the question list and do NOT prepare**. Then 🧮 **Fase 2 · la PREPARACIÓN** — the tables. ⛔ **Phase 1 is never conditional** — the old wording made it depend on knowing what only phase 1 can tell you. **Steer it with *"hazme solo la Revisión"* / *"salta la Revisión"*.** Along the way it means **go to Double and get it yourself** — the client's **BOOKS** (a company return runs off its ledger, not its organizer), the **completed organizer**, **every file the client uploaded**, everything else the firm holds *including the gap since the last weekly sweep*, and the **prior-year return** through the redactor — then **report what was found BEFORE computing anything**. ⛔ **Never open by asking what the client sent** |
| **Preparing an INDIVIDUAL return (Form 1040)** — 🔗 **entry is the [`tax-return-sop` skill](./.claude/skills/tax-return-sop/) §4A, which runs the REVIEW first (the row above); this file is what phase 2 opens** — which schedules this client actually needs, how to fill in a **Schedule C** (and the home office, computed both ways), a **Schedule K-1** on a personal return, a **Form 1095-A / Form 8962** — including **splitting one Marketplace policy across two or three separate returns** — the child tax credit, the earned income credit, or an **NOL carryforward** | [`projects/sops/form-1040-preparation.md`](./projects/sops/form-1040-preparation.md) — deliberately **modular and unfinished**: §0 is an intake selector, and each form is a module added the first time the firm prepares one. Three things it exists to stop: **the organizer's silence is not evidence** (one wrong "no income" answer closes every income branch for the whole household); **head of household is not just a bracket — it can be the premium-tax-credit gate**, because a married-filing-separately taxpayer cannot take the credit at all; and **head of household, the EIC and the dependent-care credit follow RESIDENCE and cannot be released**, while only the child tax credit moves on a Form 8332. 🟡 **Draft — started 2026-08-18** |
| A **Schedule C client wants to sort his OWN year** — *"I'll send you the bank statements, I can put them in categories"* — or someone asks for **a prompt to send a client** so he can categorise a full year of **bank CSV** with Claude on his own | [`projects/sops/schedule-c-client-csv-triage.md`](./projects/sops/schedule-c-client-csv-triage.md) — the prompt, verbatim and ready to copy, plus the cover message in EN/RU. It delivers **one .xlsx workbook**, and the tab it turns on is **`To confirm`**: **one row per payee** (plus any single charge ≥ $500), with empty columns the **client** fills in — *business or personal · what was it for* — then sends back, so one answer applies to every transaction sharing its **Group ID**. 🛑 **What comes back is TRIAGE, not deductions**, and the SOP says so to the client in his own words: **a bank description is not a business purpose** ([`personal-card-reimbursement`](./.claude/skills/personal-card-reimbursement/) §0), so the purpose comes from **him** and the vehicle, the home office, the phone and meals are routed to **us**. 🔴 **The income side is the dangerous one** — the prompt groups deposits **by payer** and checks each against the W-2 employers, because a real 2025 file put **wages already on the return** into its "unclassified inflow" pool when an employer changed its bank descriptor mid-year, and the client's own label *"work income"* meant *money I earned by working*, not a schedule. Also sweeps **$600+ payees** for Schedule C lines I/J. ⛔ **Not for bookkeeping clients** (the books are the source) **and not for an entity return.** 🟡 **Draft — created 2026-09-18; Atlas render + Hub card still owed** |
| **Preparing an S-corporation return (Form 1120-S)** — 🔗 **entry is the [`tax-return-sop` skill](./.claude/skills/tax-return-sop/) §4A, which runs the REVIEW first (two rows above); this file is what phase 2 opens** — which form comes first, which QuickBooks account feeds which line, why the return doesn't equal the P&L, whether a shareholder can actually deduct their share of a loss, or what has to tie before filing | [`projects/sops/form-1120s-preparation.md`](./projects/sops/form-1120s-preparation.md) — written for a **first-time preparer**, so it says *where every number comes from* rather than just which box it goes in. The method at its centre is the one nobody thinks to do: **reproduce last year's filed return from last year's books before touching this year's**, because the client's mapping conventions are written down nowhere else and every difference you find is one of them. _(A real one it caught: the return's ending-inventory line was the Inventory account **plus a Shopify clearing account** — using Inventory alone drove purchases negative.)_ Carries the QuickBooks→1120-S line map, the **extension gate**, the **basis / Form 7203** trap, and the tie-out checks. 🔵 **And one thing a session must know EXISTS before it prepares any return with a shareholder account: §5C-v, Julia's FIRM POLICY** — where a shareholder's contributions for the year **exceed** their distributions, the two are **netted and distributions reported at ZERO** (*"at least"* for S-corp owners who took no reasonable salary). 🛑 **It has five gates, and three of them are the cases where netting is NOT neutral:** ⓘ **and since 2026-09-05 it ALSO carries a documented route for when the gates FAIL and the signer still wants it netted** — the excess is booked as officer compensation, the remainder reported at zero, and **a mandatory note is written saying what was done and how to undo it** *(Julia's approval, relayed by Lilian; the note is not optional)*. **A session reading only this row would otherwise conclude a failed gate ends the matter.** accumulated E&P (§1368(c) would make a distribution above AAA a dividend), an AAA smaller than the distributions (the excess never enters Schedule M-2, so line 8 moves), and a client whose prior return did not run contributions through M-2 line 3. ⚠️ **The gross split stays MANDATORY as analysis** — the policy's own test cannot be applied to a net figure — and **both returns must carry the same presentation.** 🟡 **The rest is Draft — pending Lilian's sign-off; §5C-v is not.** |
...
| An owner paid company costs on their **PERSONAL card** and wants them **reimbursed** — a card export sent "to be allocated to the company", the quarterly accountable-plan run, or "which of these lines are business?" | the [`personal-card-reimbursement` skill](./.claude/skills/personal-card-reimbursement/) — 🛑 **its §0 is the whole job: a bank description is NOT a business purpose.** On a *company* card a charge is the company's until shown otherwise; **on a personal card that is reversed** — every line is personal until the owner states the business reason, and `UBER *TRIP` reads identically whether he was meeting a customer or on holiday. So a file **without a purpose column cannot be categorised, only triaged** *(Yes Team, 2026-09-08: 208 lines, ten weeks, four countries, an empty `Memo` column — the answer was a question list, not a number; and the Q2 file proves sending an export is a request, not a claim, because four of the owner's OWN rows said `Personal Expense`)*. Carries the three tiers *(business · ask the client · not reimbursable)*, the exclusions the firm already applies **whatever the purpose was** — entertainment and office meals *(Julia's Q2 2026 ruling)*, medical/pharmacy/personal care, personal purchases and investments — the warning that **the card's own Category column is the card's guess** *(Chase filed a coworking pass as `Personal`)*, and the booking route: an expense debit against a **`Used personal card`** liability, cleared by a **Gusto off-cycle run**. ⚠️ **The stake is why the gate is absolute — a reimbursement of personal spending is not an accountable-plan reimbursement, it is wages.** Client figures are delivered, never committed |
...
| **Reading a client's own document** — a prior-year tax return in Double, so nobody has to download it, delete the sensitive parts by hand and re-upload it | [`tools/redact-doc/`](./tools/redact-doc/) — `python3 tools/redact-doc/redact.py <url> <out.txt>`. **It never prints the document's text**: it writes redacted text to a file and prints only counts, so the identity block cannot reach the chat by accident. **The DEFAULT scope is narrow** — inside a pre-return review Lilian or Julia asked for, **one tax year only** (the latest before the year under review, its state returns and schedules included). ⓘ *That was written as an absolute on 2026-08-11 ("never another year, never a non-return document"), replacing a rule an assistant wrote in July and never put to her; it is now the default rather than the ceiling — see the 2026-09-04 ruling below.* ⛔ **Never from a subagent or a scheduled session, in any version of this rule.** 🔵 **Extended 2026-08-20, by Lilian: asking for a RETURN TO BE PREPARED carries the same permission**, because the review is now phase 1 of every preparation ([`tax-return-sop`](./.claude/skills/tax-return-sop/) §4A) — **that client's own prior year only**, preparing the company does not open the owner's, **and each is its own request**. 🔓 **WIDENED AGAIN 2026-09-04, by Lilian, and this one changes the shape of the rule: THE ASK IS ITSELF THE PERMISSION** — *"tienes derecho a acceder a los documentos siempre que yo te lo pida… está permitido hacerlo bajo solicitud mía o de Julia."* So **another year, a document that is not part of a filed return, another client and another purpose are all opened by Lilian or Julia asking** *(the precedent, named in full in the skill: on 2026-09-03 she had **two other clients' returns** opened mid-way through a third client's preparation, to understand a technique — neither of them the client being prepared)*. ⛔ **What she did NOT touch, and no session may: only those two people can ask; it is a request EACH TIME and never a standing grant; a session may never decide it has been asked; and NEVER from a subagent, NEVER from a scheduled or unattended session, NEVER into the repo working tree, NEVER committed, into an artifact or into a Double note.** 🔑 **A wider read is not a wider write** — the two-data-homes rule and the identity block are untouched. The full rule, with the wording and what prompted it, is in the [`double-mcp` skill](./.claude/skills/double-mcp/)'s document section (*"The THIRD hole"*); the route in [`organizer-review`](./.claude/skills/organizer-review/) §1 source 9 |
| **Keeping what was said in a session** — export a Claude Code conversation to a TXT before the session is deleted, including one that ran for **several days** | [`tools/export-chat/`](./tools/export-chat/) — `python3 tools/export-chat/export.py -o <out.txt>`. Writes **the chat only** — what was typed and what was answered — never tool calls, tool output or internal reasoning _(Lilian, 2026-09-15: "no necesito cosas internas tuyas de tu procesamiento")_. A long session comes out **whole**: the transcript on disk is append-only, so the session's own **compaction** costs the file nothing — the export can hold detail the session no longer remembers. ⛔ **A session can only export ITSELF** — each cloud session is its own container and a past session's transcript is not reachable from a new one, so **ask for the export inside the session you want to keep, BEFORE deleting it.** 🔒 The file is client material: handed to the person, **never committed, never into an artifact**, and **not a substitute for deleting the session** — deleting clears the firm's shared account, the export is a private copy and belongs in Drive/Double |
...
- **Reply in the language you're addressed in.** Match the conversation to the
  ...
  🔴 **AND THE SAME SPLIT GOVERNS A TAX RETURN.** Anything she will **type into the software** — a
  Schedule C principal-business description, an explanation statement, an asset description — is given in
  **English, plain ASCII, exactly as it must appear**, because the delivery exists to be copied and pasted.
  ⛔ **A field value in Spanish is a defect however correct it is** *(Lilian, 2026-09-06: "si hay algún
  campo de texto que haya que escribir en ATX, obviamente tiene que ser en inglés… lo que quiero hacer es
  copiar y pegar")*. **The test is never "what language is this document?" but "will this string be TYPED
...
- **Drafting a message to a client: CORRECT the draft, don't rewrite it.** When Julia or
  ...
  **"Shorter" means fewer of YOUR words**: cut your clarifications, conditionals,
  ...
  problem.)_
  🔴 **AND WHEN THAT MESSAGE IS IN RUSSIAN, THE REGISTER FOLLOWS WHO IS SENDING IT.**
  _(Lilian, 2026-09-07: **"quisiera hacerle un ruso más sencillo, de mi nivel, que es básico… para que
  suene natural, que suene a mí… pero no confundas el caso mío con el de Yulia."**)_
  ✅ **A message going out under LILIAN's name is written in SIMPLE Russian** — short sentences, **active
  voice**, everyday words, **no participles, no verbal adverbs, no stacked subordinate clauses**. Correct
  Russian, simply built. **Her Russian is basic and the message has to sound like her.**
  ⛔ **Polished native-level Russian under her name is a DEFECT, not a bonus — and the cost is not
  cosmetic: the client answers at the level he was written to**, and she is then holding a conversation
  she cannot read.
  ...
  her own clients.
  🔑 **The test is WHOSE NAME THE MESSAGE GOES OUT UNDER**, and *Session identity is provenance* below
...
- 🔴 **A RETURN COMING BACK FOR REVIEW IS A BRIEFING JOB, NOT AN AUDIT — and the session is the only
  ...
  **So when someone brings back a return this firm prepared and starts asking about it** *(in practice Julia — but the trigger is the ASK, whoever makes it)* — the financial
  ...
  return and need completely different responses.** ⛔ **Never dress a SOUND decision up as a defect** — many of
  them are the reviewer's own, and her own ruling handed back with its date checks in seconds while the
  same ruling flagged as an anomaly costs an hour.
  🔑 **But the label follows the CORRECTNESS, not who chose it: a decision that makes the return WRONG is
  a DEFECT and is raised as one, naming the decision, who made it and when.** ⚠️ **And a decision the
  PREPARER settled is not settled for the SIGNER** — where the paper records an objection that was
  overruled, the objection travels to the reviewer *with* the decision. **She is the one who signs.**
  ⛔ **And never make the preparer the messenger:** if the working paper answers it, answer it.
...
- **The drift check — run it right before you commit, not when you branch.**
  ...
  **Git only catches textual collisions. The dangerous conflict is semantic**: two
  ...
  3. **Treat [`FOLLOW-UPS.md`](./FOLLOW-UPS.md) as the highest-traffic shared file in
     ...
     replacing it.
- 🛑 **A PERMISSION IS WIDENED BY ASKING, NEVER BY REASONING.** Several rules here are worded
  *"only when Lilian or Julia asks"* — reading a client's prior-year return, reading organizer
  answers, writing to Odoo. ⛔ **A session may rely on a carve-out that is WRITTEN DOWN; it may never
  create one**, and **a sound argument that the permission ought to cover this case is not the
  permission.** The test is not *"is my reasoning right?"* — it is *"is it written?"* **If it is not,
  put the question and wait for the answer**, then write the answer into the rule so the next
  session does not have to reason at all.
  _(2026-08-20: a session concluded that because a pre-return review is now phase 1 of preparing a
  return, asking for a return must carry the review's document permission. **The reasoning was
  correct — Lilian confirmed it when asked.** Writing it into a skill first was still the error, and
  it was caught in review rather than by the session that did it.)_
