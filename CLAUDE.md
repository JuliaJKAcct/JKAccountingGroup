...
│       ├── alta-statement-categorization/  categorize a real estate purchase/sale ALTA settlement statement into the HUD tool + a Schedule D number
...
| A **real estate closing** — a purchase or sale **ALTA settlement statement** (or HUD-1 / Closing Disclosure) to categorize, prefill into the HUD tool, tie out, or turn into a **Schedule D / Form 4797** gain-or-loss and a journal entry; also folding a flip's **carrying costs** from the bank ledger into basis, or reading a **scanned** ALTA | the [`alta-statement-categorization` skill](./.claude/skills/alta-statement-categorization/) — the four buyer buckets (facilitative → basis, loan cost → amortized, operating → expensed, escrow → asset) and the seller's selling costs that reduce the amount realized; carries the blank HUD template, the standing mapping chart, and a verifier that **refuses to ship a workbook that doesn't tie to the penny**. Client figures delivered, never committed |
