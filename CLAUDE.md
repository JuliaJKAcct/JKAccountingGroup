...
| A client has a **brokerage / managed investment account** — the monthly journal entry from its statements, the **year-end split of realized vs unrealized** gain, or how dividends, capital gains and the unrealized movement land on the **tax return** | [`projects/sops/brokerage-investment-accounts.md`](./projects/sops/brokerage-investment-accounts.md) — an **every-year** procedure whose year-end half is the part that gets forgotten. 🔑 **The idea it turns on: the books carry the account at MARKET value, so the monthly entry's balancing line absorbs REALIZED and UNREALIZED movement in one number — and only one of the two is taxable.** Splitting them needs three documents (the **1099 per account**, the **December statement**, the **prior-year return**) and is impossible from the books alone. Carries the **five ties** that prove the year's bookkeeping first, the split and its cross-check, the single reclass entry, and the return treatment. 🔴 **§6 exists because a real filed return got three things wrong or nearly so:** the entire dividend total went to Schedule C's **"Other dividends"** line, which carries **no percentage and therefore NO dividends-received deduction** — for a C-corp holding domestic equities that is money left on the table, gated by **§854(b) eligibility the 1099 does not report** and by **§246(c)'s 46-day holding period**, which bites hardest on a portfolio rebuilt late in the year; a **corporate capital loss deducts NOTHING** against ordinary income (§1211(a)) and its carryforward **leaves no trace on the filed return**, so the firm's own register is the only record; and 🛑 **the Schedule M-1 SIGN TRAP** — a book unrealized **LOSS** goes on **line 5**, a **GAIN** on **line 7**, so copying last year's line placement produces a return that balances internally and is wrong. 📗 **And the worksheet that runs it is [`projects/sops/assets/Form-1120-Preparation-Worksheet.xlsx`](./projects/sops/assets/Form-1120-Preparation-Worksheet.xlsx)** — one Excel workbook per C-corporation per year, fourteen tabs, the whole 1120 from the trial balance to the tie-outs. **It enforces all three of the above in formulas**: Schedule C computes **no DRD until §246(c) is answered** and warns when the whole dividend total sits on the catch-all line while the client holds domestic shares; Schedule D **floors a net capital loss to zero** on page 1 and carries the loss pool; and Schedule M-1 takes **one signed figure** and chooses line 5 or line 7 **itself**. ➕ **And a fourth, added 2026-09-10 after the reviewer caught it on a live return: Form 8949 COLUMN (g).** Schedule D is `(h) = (d) − (e) + (g)`, and **(g) is where the wash sales go** — the 1099-B's gain column is already net of them, so a sheet built from *proceeds · basis · gain* has **every figure right and still does not add up**, off by exactly the disallowed amount. The tab now takes (g) as an input, **computes (h)**, and carries a **PROOF block reading PASS / CHECK** under each part *(SOP §6C-i)*. Verified end to end against a real year — every tie passes and M-1 closes to page 1 line 28 to the cent. Figures for one client go in [`projects/tax-returns/`](./projects/tax-returns/) and in a copy of the workbook — never in the SOP, and the **blank master is the only version committed** |
