...
│   ├── hiring-bookkeeper-test/    the PRACTICAL BOOKKEEPER HIRING TEST — mock QuickBooks books
│   │                              (fictional client) with 21 seeded errors, the bank statements,
│   │                              the candidate brief and the evaluator answer key; generated from
│   │                              code that asserts its own tie-outs (Active)
...
| **Hiring a bookkeeper** — a practical test to put in front of a candidate, mock QuickBooks books to reconcile, a set of seeded errors, the scoring rubric, or the answer key for an interview already scheduled | [`projects/hiring-bookkeeper-test/`](./projects/hiring-bookkeeper-test/) — read [`test-package/00_START_HERE.md`](./projects/hiring-bookkeeper-test/test-package/00_START_HERE.md) first. Three months of a fictional construction company with **21 deliberate errors** across the bank reconciliation, the month-end close and the financial statements. ⛔ **`ANSWER_KEY_evaluator_only.html` never goes to a candidate**, and the error count is never disclosed — the count is the test. 🔑 **The books are GENERATED, never hand-edited**: change [`build/books.py`](./projects/hiring-bookkeeper-test/build/) and re-run, because `engine.py` asserts the balance sheet balances, the sub-ledgers tie to their control accounts, each month's gross margin hits target and the reconciliation resolves to the cent — a hand-edit breaks a tie-out silently. ⚠️ **Seed a fresh variant per candidate** once answers start circulating. All mock data; no client information may enter it |
