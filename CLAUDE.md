...
| **What a monthly plan costs and what's in it** — the pricing page, which plan a client belongs on, whether something is included or an add-on, or how to justify an upgrade | [`projects/marketing/service-packaging.md`](./projects/marketing/service-packaging.md) is the authority. Three plans (**Essential · Advisory · Strategic**) sold as **depth of relationship**, never as quantities of bookkeeping, reports or response time. The three rules a session gets wrong: **communication is in every plan and is never a tier** — the old "Response Time 96/48/24 hours" grid is gone and reply-within-one-business-day is a promise printed *above* the plans, so what climbs is **who starts the conversation**, not how fast we answer; **no allowance is ever unbounded** — employee payroll runs **none on Essential · up to 5 on Advisory and Strategic**, and above five we set the client up on a platform and supervise rather than run it, with 1099s, states and entities bounded the same way (**cadence** — weekly vs biweekly — is the client's choice and never a tier, which is what the live page gets backwards); and **bill pay, invoicing & collections and controller/CFO work are add-ons, not plan contents** — *Strategic includes the conversation, the Controller add-on includes the deliverable* (talking through a loan is Strategic; building the bank's financial package is paid controller work). Owner payroll is in **every** plan — it is the S-corp requirement, and it is $0 internally |
