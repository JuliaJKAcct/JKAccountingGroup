...
| Skill | Reach for it when… |
|---|---|
| [`tax-return-sop`](./tax-return-sop/) | Writing/extending a **TAX-RETURN SOP** — the form-by-form procedure for one kind of return (`form-1120s-preparation.md` is the reference; one per form is the goal) — **and using one to drive a real return**, producing the line-by-line tables a first-time preparer can work from. Why these differ from every other SOP (they must say *where each number comes from*), the required spine, build-the-map-from-the-prior-year, the **five-part delivery standard** (a table per form · the order of preparation · the flow of figures between forms · the checkboxes with their reasons · the explanations), the 🛑 **pull-the-PDF-from-irs.gov** rule for any answer that changes a figure, and the working paper every return leaves behind. Builds on [`sop-authoring`](./sop-authoring/). |
