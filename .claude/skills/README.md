...
| [`alta-statement-categorization`](./alta-statement-categorization/) | Categorizing a **real estate purchase/sale ALTA settlement statement** — every closing line into its bucket (facilitative → basis, loan cost → amortized, operating → expensed, escrow → asset; seller-side selling costs reduce the amount realized), prefilled into the HUD tool, plus the **Schedule D / Form 4797** gain-or-loss and the disposition journal entry. Also folds a flip's **carrying costs** from the bank ledger into basis (tiered A–D) and reads **scanned** ALTAs. *(Every reconciliation must tie to the penny — the verifier exits non-zero otherwise. Client figures delivered, never committed.)* |
