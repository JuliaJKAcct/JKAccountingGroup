...
| **§1 · Gather this before you start** | ⚠️ **Half the list is about the prior year.** Always ask for the **general ledger**, always ask for **gross** figures where an account nets things, always **check the basis printed on every export** — and ⛔ **never resolve a books-versus-return basis mismatch by re-exporting on the other basis**: a package's cash toggle cannot change what the entity elected, it leaves journal-entry accruals behind, and it hides the liabilities the M-1 is found from |
