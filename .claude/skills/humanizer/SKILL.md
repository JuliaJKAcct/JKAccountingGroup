...
## JK Accounting Group — local notes (not part of the upstream skill)

Vendored from [blader/humanizer](https://github.com/blader/humanizer) (MIT, © 2025 Siqi
Chen), v2.11.1, on 2026-08-19 at Julia's request. LICENSE sits beside this file. To
update, diff against the upstream SKILL.md and keep this section.

- **Standing use:** run this skill over every client-facing text the firm produces —
  proposals, engagement letters, client emails, website copy — before delivery. Julia's
  trigger for installing it was em dashes in a proposal draft (§14 is the pattern she
  spotted).
- **Two carve-outs, both are the "writer's voice outranks the rules" clause upstream:**
  the **brand's fixed display labels** (e.g. the proposal tools' "The foundation — always
  included") are house design and keep their em dashes — humanize the per-client prose,
  not the shared templates; and **Russian text keeps grammatically required тире** (the
  predicative dash is correct Russian, not an AI tell) — what gets removed in Russian is
  the English-style parenthetical dash pair and dash-as-list-glue, where a colon or comma
  serves.
